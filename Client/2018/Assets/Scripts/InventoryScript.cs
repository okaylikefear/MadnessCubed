...
public class InventoryScript : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			this.weaponParams[i].id = i;
		}
		for (int j = 0; j < this.itemDesc.Length; j++)
		{
			this.itemDesc[j].id = j;
		}
	...
	private void Start()
	{
		...
		this.shopFace = this.collectClothes(InventoryScript.ClothesPages.Face);
	...
	private void ToggleInventar()
	{
		...
		if (!Kube.OH.isMenu)
		{
			if (Kube.BCS.gameProcess != BattleControllerScript.GameProcess.end)
			{
				...
				{
					monoBehaviour = Cub2Menu.Find<MonoBehaviour>("Decor_menu");
				}
				else
				{
					monoBehaviour = Cub2Menu.Find<MonoBehaviour>("Arsenal_menu");
				}
	...
	private void ToggleinventarHealth()
	{
		...
		if (!Kube.OH.isMenu)
		{
			...
			{
				Kube.BCS.menu.SetActive(true);
				Cub2UI.FindAndOpenMenu<MonoBehaviour>("Decor_menu");
			}
		}
		else
		{
	...
	public void ChoseFastInventarKey(int num)
	{
		if (num < 0)
		{
			num = 10 + num;
	...
	public void ChoseFastInventar(int num)
	{
		...
		else if (Kube.BCS.ps.fastInventar[num].Type != 4)
		{
			if (this.ps != null)
			{
				...
				if (Kube.BCS.ps.fastInventar[num].Type == 7)
				{
					this.ps.DoUseSpec(Kube.BCS.ps.fastInventar[num].Num);
				}
				else
				{
	...
	{
		for (int i = 0; i < Kube.GPS.fastInventarCreating.Length; i++)
		{
			if (Kube.GPS.fastInventarCreating[i].Type == 3 && Kube.GPS.inventarItems[Kube.GPS.fastInventarCreating[i].Num] <= 0)
			{
				Kube.GPS.fastInventarCreating[i].Type = -1;
				Kube.GPS.fastInventarCreating[i].Num = 0;
			}
		}
	...
	{
		FastInventar[] array = Kube.GPS.fastInventarCreating;
		if (type == 1)
		{
		...
		if (item.Type == 4)
		{
			int weaponGroup = (int)Kube.IS.weaponParams[item.Num].weaponGroup;
			array[weaponGroup] = item;
			Kube.SS.SaveFastInventory(type, array, null);
			return weaponGroup;
		}
	...
	private void BuyCubesDone(string[] strs)
	{
		if (strs[0] == "0")
		{
			...
			{
				Kube.GPS.cubesTimeOfEnd[num2] = (int)Time.time + (Convert.ToInt32(array[num2]) - num);
				num2++;
			}
	...
	public void BuyVIPSubsDone(string ans)
	{
	}
	...
	public int UseItem(int itemNum)
	{
		...
		if (Kube.GPS.inventarItems[itemNum] <= 0)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				if (Kube.GPS.fastInventarCreating[i].Type != 7)
				{
					if (Kube.GPS.fastInventarCreating[i].Type != 4)
					{
						if (Kube.GPS.fastInventarCreating[i].Num == itemNum)
						{
							Kube.GPS.fastInventarCreating[i].Type = -1;
							Kube.GPS.fastInventarCreating[i].Num = 0;
						}
	...
	public bool canPay(FastInventar fi)
	{
		if (fi.Type == 3)
		{
			return Kube.GPS.inventarItemPrice2[fi.Num] <= Kube.GPS.playerMoney2;
		}
		return Kube.GPS.specItemsPrice2[fi.Num, 0] <= Kube.GPS.playerMoney2;
	}
	...
	public ItemDescObj findBox(CaseType video)
	{
		for (int i = 0; i < this.itemDesc.Length; i++)
		{
			if (this.itemDesc[i].caseType == video && this.itemDesc[i].page == InventoryScript.ItemPage.Boxes)
			{
				return this.itemDesc[i];
			}
		}
		return null;
	}
	...
	public SkinDescObj[] skins;

	public DressDescObj[] dressItems;

	public WeaponParamsObj[] weaponParams;

	public WSkinDescObj[] weaponSkins;

	public ItemDescObj[] itemDesc;

	public ItemDescObj[] specItemDesc;
