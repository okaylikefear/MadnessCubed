...
public class ResourceManagerBase : MonoBehaviour
{
	...
	public void Init(string assetPath, string bundlesPath)
	{
		if (this.initialized)
		{
			return;
	...
	public void DrawLoading()
	{
		...
		if (!this.isDownload)
		{
			Vector3 vector = new Vector3((float)Screen.width / 1000f, (float)Screen.height / 600f, 1f);
			num = (float)KUI.width;
			num2 = (float)KUI.height;
			GUI.DrawTexture(new Rect(0.5f * (num - 318f), num2 - 100f, 318f, 25f), this.pb_bgTex);
			float width = 318f / (float)this.pb_runTex.width;
			GUI.DrawTextureWithTexCoords(new Rect(0.5f * (num - 318f), num2 - 100f, 318f, 25f), this.pb_runTex, new Rect(-Time.time, 0f, width, 1f));
