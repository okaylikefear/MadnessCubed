...
public class ActionAreaScript : GameMapItem
{
	...
	private void UpdateHorizontalDoor()
	{
		...
		for (int m = 0; m < Mathf.Abs(this.x2 - this.x1) + 1; m++)
		{
			for (int n = 0; n < Mathf.Abs(this.z2 - this.z1) + 1; n++)
			{
				...
				{
					if (m + this.x1 <= Kube.WHS.sizeX)
					{
						if (n + this.z1 <= Kube.WHS.sizeY)
						{
							if (array[m, n])
							{
								Kube.WHS.prop[m + this.x1, num, n + this.z1] = 1;
							}
							else
							{
								Kube.WHS.prop[m + this.x1, num, n + this.z1] = 0;
							}
						}
					}
				}
	...
	private void GenerateAAMesh(Mesh mesh, Vector3 pos1, Vector3 pos2, float uvScaleOffset, Vector3 parentPos)
	{
		...
		for (int i = 0; i < num; i++)
		{
			for (int j = 0; j < num2; j++)
			{
				...
				{
					Vector3 b = new Vector3((float)i, (float)j, (float)k);
					for (int l = 0; l < 6; l++)
					{
						bool flag = false;
						if (j == num2 - 1 && l == 0)
						...
						if (flag)
						{
							array3[num5] = array2[l, 0] + b;
							array3[num5 + 1] = array2[l, 1] + b;
							array3[num5 + 2] = array2[l, 2] + b;
							array3[num5 + 3] = array2[l, 2] + b;
							array3[num5 + 4] = array2[l, 3] + b;
							array3[num5 + 5] = array2[l, 0] + b;
							int num9 = Kube.OH.blockTypes[this.materialType].itemId;
							if (num9 < 0)
							{
	...
	private void RecreateSamples()
	{
				...
				for (int k = (int)vector.z; k <= (int)vector2.z; k++)
				{
					if (Kube.WHS.IsInWorld(i, j, k))
					{
						if (Kube.WHS.cubeTypes[i, j, k] != 0)
						{
							...
							if (num2 >= 4096)
							{
								Kube.WHS.ChangeCubes(Kube.OH.GetServerCode(num2, 2) + text, false, true);
								num2 = 0;
								text = string.Empty;
							}
						}
		...
		if (this.type == AAType.doorVertical)
		{
			...
			for (int m = (int)vector2.y; m >= (int)vector.y; m--)
			{
				...
				int num4 = Kube.OH.blockTypes[this.materialType].atlas;
				if (num4 < 0)
				{
					num4 = 0;
				}
				this.AAsamples[num3].GetComponent<Renderer>().material = Kube.ASS3.cubesMat[num4];
				this.GenerateAAMesh(this.AAsamples[num3].GetComponent<MeshFilter>().mesh, new Vector3(vector.x - 0.5f, (float)m - 0.5f, vector.z - 0.5f), new Vector3(vector2.x + 0.5f, (float)m + 0.5f, vector2.z + 0.5f), 0.5f, this.AAsamples[num3].transform.position);
		...
		if (this.type == AAType.doorHorizontal)
		{
			if (this.prop2 == 0)
			{
				...
				{
					this.AAsamples[num5] = (UnityEngine.Object.Instantiate(this.AAsimplePrefab, new Vector3((float)n, vector.y, vector.z), Quaternion.identity) as GameObject);
					int atlas = Kube.OH.blockTypes[this.materialType].atlas;
					this.AAsamples[num5].GetComponent<Renderer>().material = Kube.ASS3.cubesMat[atlas];
					this.GenerateAAMesh(this.AAsamples[num5].GetComponent<MeshFilter>().mesh, new Vector3((float)n - 0.5f, vector.y - 0.5f, vector.z - 0.5f), new Vector3((float)n + 0.5f, vector2.y + 0.5f, vector2.z + 0.5f), 0.5f, this.AAsamples[num5].transform.position);
			...
			else if (this.prop2 == 1)
			{
				...
				int num6 = 0;
				for (int num7 = (int)vector2.z; num7 >= (int)vector.z; num7--)
				{
					this.AAsamples[num6] = (UnityEngine.Object.Instantiate(this.AAsimplePrefab, new Vector3(vector.x, vector.y, (float)num7), Quaternion.identity) as GameObject);
					int atlas2 = Kube.OH.blockTypes[this.materialType].atlas;
					this.AAsamples[num6].GetComponent<Renderer>().material = Kube.ASS3.cubesMat[atlas2];
					this.GenerateAAMesh(this.AAsamples[num6].GetComponent<MeshFilter>().mesh, new Vector3(vector.x - 0.5f, vector.y - 0.5f, (float)num7 - 0.5f), new Vector3(vector2.x + 0.5f, vector2.y + 0.5f, (float)num7 + 0.5f), 0.5f, this.AAsamples[num6].transform.position);
					this.AAsamples[num6].GetComponent<MeshCollider>().sharedMesh = null;
					this.AAsamples[num6].GetComponent<MeshCollider>().sharedMesh = this.AAsamples[num6].GetComponent<MeshFilter>().mesh;
					num6++;
				}
			}
			else if (this.prop2 == 2)
			{
				...
				int num8 = 0;
				for (int num9 = (int)vector.x; num9 <= (int)vector2.x; num9++)
				{
					this.AAsamples[num8] = (UnityEngine.Object.Instantiate(this.AAsimplePrefab, new Vector3((float)num9, vector.y, vector.z), Quaternion.identity) as GameObject);
					int atlas3 = Kube.OH.blockTypes[this.materialType].atlas;
					this.AAsamples[num8].GetComponent<Renderer>().material = Kube.ASS3.cubesMat[atlas3];
					this.GenerateAAMesh(this.AAsamples[num8].GetComponent<MeshFilter>().mesh, new Vector3((float)num9 - 0.5f, vector.y - 0.5f, vector.z - 0.5f), new Vector3((float)num9 + 0.5f, vector2.y + 0.5f, vector2.z + 0.5f), 0.5f, this.AAsamples[num8].transform.position);
					this.AAsamples[num8].GetComponent<MeshCollider>().sharedMesh = null;
					this.AAsamples[num8].GetComponent<MeshCollider>().sharedMesh = this.AAsamples[num8].GetComponent<MeshFilter>().mesh;
					num8++;
				}
			}
			else if (this.prop2 == 3)
			{
				...
				this.maxHeight = vector.x - vector2.x;
				int num10 = 0;
				for (int num11 = (int)vector.z; num11 <= (int)vector2.z; num11++)
				{
					this.AAsamples[num10] = (UnityEngine.Object.Instantiate(this.AAsimplePrefab, new Vector3(vector.x, vector.y, (float)num11), Quaternion.identity) as GameObject);
					int atlas4 = Kube.OH.blockTypes[this.materialType].atlas;
					this.AAsamples[num10].GetComponent<Renderer>().material = Kube.ASS3.cubesMat[atlas4];
					this.GenerateAAMesh(this.AAsamples[num10].GetComponent<MeshFilter>().mesh, new Vector3(vector.x - 0.5f, vector.y - 0.5f, (float)num11 - 0.5f), new Vector3(vector2.x + 0.5f, vector2.y + 0.5f, (float)num11 + 0.5f), 0.5f, this.AAsamples[num10].transform.position);
					this.AAsamples[num10].GetComponent<MeshCollider>().sharedMesh = null;
					this.AAsamples[num10].GetComponent<MeshCollider>().sharedMesh = this.AAsamples[num10].GetComponent<MeshFilter>().mesh;
					num10++;
				}
			}
		}
		if (this.type == AAType.lift)
		{
			...
			for (int num12 = 0; num12 < componentsInChildren.Length; num12++)
			{
				componentsInChildren[num12].gameObject.transform.position = Vector3.Lerp(vector, vector2, 0.5f);
				componentsInChildren[num12].gameObject.transform.localScale = vector2 - vector + Vector3.one;
				componentsInChildren[num12].emissionRate = (float)((int)(((vector2 - vector).x + 1f) * ((vector2 - vector).y + 1f) * ((vector2 - vector).z + 1f)));
				if (componentsInChildren[num12].gameObject.name == "on")
				{
					componentsInChildren[num12].emissionRate *= 3f;
				}
			}
		}
		if (this.type == AAType.forceField)
		{
			...
			this.AAsamples[0].GetComponent<Renderer>().sharedMaterial = Kube.OH.AAselectMat;
			this.AAsamples[0].GetComponent<Renderer>().enabled = (Kube.BCS.gameMode == GameMode.creating);
	...
	private void SetParameters(int playerId)
	{
		...
		this.materialType = Kube.WHS.cubeTypes[this.x1, this.y1, this.z1];
		if (this.id == -1)
		{
		...
		this.NO.CreateNewAA(this.x1, this.y1, this.z1, this.x2, this.y2, this.z2, (int)this.type, this.materialType, this.status, (int)(this.coordState * 255f), this.soundType, this.prop1, this.prop2, this.prop3, this.id, Kube.BCS.onlineId);
	...
	{
		bool flag = false;
		if (_x1 > Kube.WHS.sizeX)
		{
			_x1 = 0;
		}
		if (_z1 > Kube.WHS.sizeZ)
		{
			_z1 = 0;
		}
		if (_x2 > Kube.WHS.sizeX)
		{
			_x2 = 0;
		}
		if (_z2 > Kube.WHS.sizeZ)
		{
			_z2 = 0;
		}
		if (this.x1 != _x1)
		...
		if (Kube.BCS != null && Kube.BCS.gameType != GameType.creating && Kube.BCS.gameType != GameType.test)
		{
	...
	private void RecreateBoundMesh()
	{
		...
		base.GetComponent<Renderer>().sharedMaterial = Kube.OH.AAselectMat;
	}
	...
	private void SaveAA(bool redraw)
	{
		...
		this.NO.SetAAParameters(num, num2, num3, this.x2, this.y2, this.z2, (int)this.type, this.materialType, this.status, (int)(this.coordState * 255f), this.soundType, this.prop1, this.prop2, this.prop3, this.id, Kube.BCS.onlineId);
	}
	...
	private void Command_On()
	{
		...
		this.NO.SetAAParameters(this.x1, this.y1, this.z1, this.x2, this.y2, this.z2, (int)this.type, this.materialType, 1, (int)(this.coordState * 255f), this.soundType, this.prop1, this.prop2, this.prop3, this.id, Kube.BCS.onlineId);
	}
	...
	private void Command_Off()
	{
		...
		this.NO.SetAAParameters(this.x1, this.y1, this.z1, this.x2, this.y2, this.z2, (int)this.type, this.materialType, 0, (int)(this.coordState * 255f), this.soundType, this.prop1, this.prop2, this.prop3, this.id, Kube.BCS.onlineId);
	}
	...
	private void Command_Toggle()
	{
		...
		this.NO.SetAAParameters(this.x1, this.y1, this.z1, this.x2, this.y2, this.z2, (int)this.type, this.materialType, 1 - this.status, (int)(this.coordState * 255f), this.soundType, this.prop1, this.prop2, this.prop3, this.id, Kube.BCS.onlineId);
	}
