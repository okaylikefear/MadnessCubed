...
public class PreLab : MonoBehaviour
{
	...
	private void UVCube()
	{
		...
		foreach (int num in sharedMesh.triangles)
		{
			Vector3 vector = normals[num];
			Vector2 vector2 = default(Vector2);
			if (Mathf.Abs(vector.x) > 0.8f)
			{
				vector2.x = vertices[num].y + 0.5f;
				vector2.y = -(vertices[num].z + 0.5f);
			}
			if (Mathf.Abs(vector.y) > 0.8f)
			{
				vector2.x = vertices[num].x + 0.5f;
				vector2.y = -(vertices[num].z + 0.5f);
			}
			if (Mathf.Abs(vector.z) > 0.8f)
			{
				vector2.x = vertices[num].x + 0.5f;
				vector2.y = -(vertices[num].y + 0.5f);
			}
			array[num] = vector2;
		}
		sharedMesh.uv = array;
