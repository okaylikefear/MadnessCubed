...
namespace kube.data
{
	public class DataUtils
	{
		...
		public static DateTime UnitToDatetime(int unixtime)
		{
			DateTime dateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0);
			return dateTime.AddSeconds((double)unixtime);
		}

		public static void CopyFileds(object src, object dst)
		{
			FieldInfo[] fields = src.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public);
			FieldInfo[] fields2 = dst.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public);
			foreach (FieldInfo fieldInfo in fields)
			{
				object obj = fieldInfo.GetValue(src);
				foreach (FieldInfo fieldInfo2 in fields2)
				{
					if (!(fieldInfo2.Name != fieldInfo.Name))
					{
						if (obj is Array)
						{
							obj = ((ICloneable)obj).Clone();
						}
						fieldInfo2.SetValue(dst, obj);
					}
				}
			}
		}
