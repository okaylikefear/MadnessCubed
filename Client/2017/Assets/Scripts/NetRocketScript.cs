...
public class NetRocketScript : Photon.MonoBehaviour
{
	...
	private void Start()
	{
		...
		this.cameraComp = Kube.IS.ps.cameraComp;
	}
	...
	private void Update()
	{
		if (base.photonView.isMine)
		{
			...
			if (Physics.Raycast(ray, out raycastHit, 1000f, 64769))
			{
				Vector3 forward = Vector3.Lerp(base.transform.TransformDirection(Vector3.forward), raycastHit.point - base.transform.position, 5f * Time.deltaTime);
				base.transform.rotation = Quaternion.LookRotation(forward);
			}
			base.transform.position += base.transform.TransformDirection(Vector3.forward * this.flySpeed * Time.deltaTime);
