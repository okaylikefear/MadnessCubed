...
public class SlotItem : MonoBehaviour
{
	private IEnumerator _loadTx()
	{
		...
		{
			this.tx.mainTexture = Kube.OH.gameItemsTex[this._invItem.Num];
			this.itemname = Localize.gameItemsNames[this._invItem.Num];
			this.UpdateCount();
		}
		else if (this._invItem.Type == 7)
		{
			this.tx.mainTexture = Kube.ASS2.specItemsInvTex[this._invItem.Num];
			this.itemname = Localize.specItemsName[this._invItem.Num];
			if (this.cnt)
			{
				this.cnt.alpha = 0f;
			}
	...
	public void UpdateCount()
	{
		...
		{
			int num = Kube.GPS.inventarItems[this._invItem.Num];
			if (Kube.BCS && Kube.BCS.ps)
			{
				num = Kube.BCS.ps.itemCnt(this._invItem.Num, num);
			}
			if (this.cnt)
			{
				if (this._invItem.Type == 4)
				{
					this.cnt.alpha = 0f;
				}
				else
