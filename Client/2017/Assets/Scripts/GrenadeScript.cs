...
public class GrenadeScript : RocketScript
{
	...
	private void Explode()
	{
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			{
				array[j].gameObject.GetComponent<Rigidbody>().AddForceAtPosition(0.01f * (float)this.dm.damage * (array[j].transform.position - base.transform.position).normalized, base.transform.position, ForceMode.Impulse);
			}
		}
