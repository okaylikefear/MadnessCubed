...
public class TutorialScript : MonoBehaviour
{
	private void PlaySoundTutor(int numOfTutor)
	{
		...
		if (numOfTutor < this.soundTutor.Length && this.soundTutor[this.currentNumOfTutor - 1] != null)
		{
			...
			base.Invoke("MusicMuteOff", this.soundTutorGO.GetComponent<AudioSource>().clip.length);
		}
	}
	...
	private void ChangeState(int numOfTutor)
	{
		if (this.currentNumOfTutor < numOfTutor)
		{
			...
			if (numOfTutor < 2)
			{
				for (int i = 0; i < 10; i++)
				{
					this.fastInventar[i].Type = -1;
					this.fastInventar[i].Num = 0;
			...
			if (numOfTutor == 27)
			{
				for (int j = 0; j < 10; j++)
				{
					this.fastInventar[j].Type = 0;
					this.fastInventar[j].Num = Kube.IS.cubesNatureNums[j];
				}
				this.timeOfNightBeginning = Time.time;
	...
	private void Start()
	{
		...
		string b = LocaleEnum.en_US.ToString();
		...
		for (int j = 0; j < this.tutorialSounds.Length; j++)
		{
			if (this.tutorialSounds[j].locale == b)
			{
				this.soundTutor = this.tutorialSounds[j].soundTutor;
				break;
			}
		}
	}
	...
	private void Update()
	{
		...
		{
			Kube.WHS.SetDayLight(1f - (Time.time - this.timeOfNightBeginning) * 0.2f, true);
		}
		if (this.currentNumOfTutor == 27)
		{
			Kube.WHS.SetDayLight((Time.time - this.timeOfNightBeginning) * 0.2f, true);
		}
	}
