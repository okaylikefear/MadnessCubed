...
public class AntiRollScript : MonoBehaviour
{
	private void FixedUpdate()
	{
		...
		if (groundHit)
		{
			GetComponent<Rigidbody>().AddForceAtPosition(this.WheelL.transform.up * -num3, this.WheelL.transform.position);
		}
		if (groundHit2)
		{
			GetComponent<Rigidbody>().AddForceAtPosition(this.WheelR.transform.up * num3, this.WheelR.transform.position);
		}
	}
