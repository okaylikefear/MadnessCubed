...
public class PathFinderMoveScript : MonoBehaviour
{
	public PathFinderMoveScript()
	{
		int[] array = new int[6];
		array[0] = 1;
		array[2] = -1;
		...
		this.doorOpenDeltaTime = 0.5f;
		
	}

	public void SetPathFinderParams(float _speed, float _jumpSpeed, int _charSizeY)
	{
		this.speed = _speed;
	...
	private int GetDistToTarget(int x1, int y1, int z1, int x2, int y2, int z2)
	{
		return (Mathf.Abs(x2 - x1) + Mathf.Abs(y2 - y1) + Mathf.Abs(z2 - z1)) * 10;
	}

	private int GetElementValue(int d1, int d2)
	{
		return d1 + d2;
	}

	private void FindPathToVector3Walking(Vector3 pos)
	{
		...
		for (;;)
		{
			...
			if (this.PFAS.openedArray[num4].stepNum <= this.maxPath)
			{
				...
				if (cubePhysType == CubePhys.air && cubePhysType2 == CubePhys.air && !Kube.WHS.isOccupied[Mathf.RoundToInt((float)this.PFAS.openedArray[num4].x), Mathf.RoundToInt((float)(this.PFAS.openedArray[num4].y - 1)), Mathf.RoundToInt((float)this.PFAS.openedArray[num4].z)])
				{
					...
					this.PFAS.openedArray[this.PFAS.openedArrayNum].distFromSource = this.PFAS.openedArray[num4].distFromSource + this.wt1[0];
				...
				{
					for (int k = 0; k < this.nX1.Length; k++)
					{
						int num9 = this.PFAS.openedArray[num4].x + this.nX1[k];
						int num10 = this.PFAS.openedArray[num4].y + this.nY1[k];
						int num11 = this.PFAS.openedArray[num4].z + this.nZ1[k];
						if (Kube.WHS.IsInWorld(num9, num10, num11))
						{
							CubePhys cubePhysType4 = Kube.WHS.GetCubePhysType(num9, num10 - 1, num11);
							if (this.nY1[k] == 0 && cubePhysType4 == CubePhys.air && !Kube.WHS.isOccupied[num9, num10 - 1, num11])
							{
								...
								if (!flag)
								{
									...
									if (!flag2)
									{
										...
										if (flag3)
										{
											...
											if (this.nY1[k] <= 0 || cubePhysType6 == CubePhys.water || cubePhysType6 == CubePhys.ledder)
											{
												this.PFAS.openedArray[this.PFAS.openedArrayNum].distFromSource = this.PFAS.openedArray[num4].distFromSource + this.wt1[k];
					...
					for (int num12 = 0; num12 < this.nX2.Length; num12++)
					{
						int num13 = this.PFAS.openedArray[num4].x + this.nX2[num12];
						int num14 = this.PFAS.openedArray[num4].y + this.nY2[num12];
						int num15 = this.PFAS.openedArray[num4].z + this.nZ2[num12];
						if (Kube.WHS.IsInWorld(num13, num14, num15))
						{
							CubePhys cubePhysType7 = Kube.WHS.GetCubePhysType(num13, num14 - 1, num15);
							if (this.nY2[num12] == 0 && cubePhysType7 == CubePhys.air && !Kube.WHS.isOccupied[num13, num14 - 1, num15])
							{
							...
							if (!Kube.WHS.isOccupied[num13, num14, num15])
							{
								...
								if (!flag4)
								{
									...
									if (!flag5)
									{
										...
										{
											CubePhys cubePhysType9 = Kube.WHS.GetCubePhysType(num13 - this.nX2[num12], num14 + num19, num15);
											if (cubePhysType9 == CubePhys.solid || cubePhysType9 == CubePhys.ice || Kube.WHS.isOccupied[num13 - this.nX2[num12], num14 + num19, num15])
											{
												flag6 = false;
												break;
											}
										...
										{
											CubePhys cubePhysType10 = Kube.WHS.GetCubePhysType(num13, num14 + num20, num15 - this.nZ2[num12]);
											if (cubePhysType10 == CubePhys.solid || cubePhysType10 == CubePhys.ice || Kube.WHS.isOccupied[num13, num14 + num20, num15 - this.nZ2[num12]])
											{
												flag6 = false;
												break;
											}
										...
										if (flag6)
										{
											this.PFAS.openedArray[this.PFAS.openedArrayNum].distFromSource = this.PFAS.openedArray[num4].distFromSource + this.wt2[num12];
					...
					if (num21 >= 1)
					{
						for (int num22 = 0; num22 < this.nX3.Length; num22++)
						{
							int num23 = this.PFAS.openedArray[num4].x + this.nX3[num22];
							int num24 = this.PFAS.openedArray[num4].y + this.nY3[num22];
							int num25 = this.PFAS.openedArray[num4].z + this.nZ3[num22];
							if (Kube.WHS.IsInWorld(num23, num24, num25))
							{
								...
								if (!flag7)
								{
									...
									if (!flag8)
									{
										...
										if (flag9)
										{
											this.PFAS.openedArray[this.PFAS.openedArrayNum].distFromSource = this.PFAS.openedArray[num4].distFromSource + this.wt3[num22];
										...
										{
											num23 = this.PFAS.openedArray[num4].x + this.nX3[num22] * num31;
											num24 = this.PFAS.openedArray[num4].y + this.nY3[num22] * num31;
											num25 = this.PFAS.openedArray[num4].z + this.nZ3[num22] * num31;
											if (Kube.WHS.IsInWorld(num23, num24, num25))
											{
												bool flag10 = true;
												for (int num32 = 0; num32 <= num31; num32++)
												{
													...
													{
														int num34 = this.PFAS.openedArray[num4].x + this.nX3[num22] * num32;
														int num35 = this.PFAS.openedArray[num4].y + this.nY3[num22] * num32 + num33;
														int num36 = this.PFAS.openedArray[num4].z + this.nZ3[num22] * num32;
														CubePhys cubePhysType14 = Kube.WHS.GetCubePhysType(num34, num35, num36);
														if (cubePhysType14 == CubePhys.solid || Kube.WHS.isOccupied[num34, num35, num36])
														{
															flag10 = false;
															break;
														}
		...
		{
			int num38 = num4;
			this.pathLength = this.PFAS.openedArray[num38].stepNum + 1;
			while (num38 != -1)
			{
				if (this.PFAS.openedArray[num38].parent != -1)
				{
					this.path[this.pathLength - this.PFAS.openedArray[num38].stepNum - 1] = new Vector3((float)this.PFAS.openedArray[num38].x, (float)this.PFAS.openedArray[num38].y, (float)this.PFAS.openedArray[num38].z);
				}
				num38 = this.PFAS.openedArray[num38].parent;
			}
			this.pathLength--;
		}
		else
		{
			float num39 = 9999999f;
			num4 = -1;
			for (int num40 = 0; num40 < this.PFAS.openedArrayNum; num40++)
			{
				if (!this.PFAS.openedArray[num40].cannotStop)
				{
					float num41 = (float)this.PFAS.openedArray[num40].distToTarget;
					if (num41 < num39)
					{
						num4 = num40;
						num39 = num41;
			...
			if (num42 >= 0)
			{
				this.pathLength = this.PFAS.openedArray[num42].stepNum + 1;
				while (num42 != -1)
				{
					if (this.PFAS.openedArray[num42].parent != -1)
					{
						this.path[this.pathLength - this.PFAS.openedArray[num42].stepNum - 1] = new Vector3((float)this.PFAS.openedArray[num42].x, (float)this.PFAS.openedArray[num42].y, (float)this.PFAS.openedArray[num42].z);
					}
					num42 = this.PFAS.openedArray[num42].parent;
				}
				this.pathLength--;
			}
		}
	...
	public void WalkingFollowTarget(Vector3 targetPos)
	{
		...
		if (this.pathLength > 0)
		{
			...
			int type = (int)Kube.WHS.cubes[Mathf.RoundToInt(vector.x), Mathf.RoundToInt(vector.y), Mathf.RoundToInt(vector.z)].type;
			if (type == 156 || type == 157)
			{
			...
			{
				if (Kube.WHS.cubes[Mathf.RoundToInt(vector.x), Mathf.RoundToInt(vector.y) + i, Mathf.RoundToInt(vector.z)].prop == CubeProps.closedDoor)
				{
					flag = true;
					break;
				}
			...
			else if (this.flagJumpAcross)
			{
				...
				float num = Mathf.Min((Time.time - this.flagTimeFrom) / (this.flagTimeTo - this.flagTimeFrom), 1f);
				Vector3 position = (this.flagTo + this.flagFrom) * 0.5f + Quaternion.AngleAxis(180f * num, axis) * (-a * 0.5f) + Vector3.up * this.deltaHeightTransform;
				base.transform.position = position;
			}
			else if (this.flagFall)
			{
				float num2 = Mathf.Min(Time.time - this.flagTimeFrom, this.flagTimeTo - this.flagTimeFrom);
				float d = -this.speed * num2 + Physics.gravity.y * num2 * num2 / 2f;
			...
			if (Vector3.Distance(base.transform.position, vector) < this.speed * Time.deltaTime * 2f)
			{
				...
				if (this.pathLength > 0)
				{
					float num3 = Mathf.Sqrt((this.path[this.pathLength - 1].x - this.lastPathPoint.x) * (this.path[this.pathLength - 1].x - this.lastPathPoint.x) + (this.path[this.pathLength - 1].z - this.lastPathPoint.z) * (this.path[this.pathLength - 1].z - this.lastPathPoint.z));
					if (this.path[this.pathLength - 1].y == this.lastPathPoint.y && num3 >= 2f && num3 <= (float)(this.jumpCubes + 1))
					{
						...
						this.flagTimeTo = Time.time + num3 / (2f * this.speed);
					}
					if (num3 == 0f && this.path[this.pathLength - 1].y + 1f < this.lastPathPoint.y)
					{
						this.flagFall = true;
						this.flagFrom = this.lastPathPoint;
						this.flagTo = this.path[this.pathLength - 1];
						this.flagTimeFrom = Time.time;
						float num4 = this.lastPathPoint.y - this.path[this.pathLength - 1].y;
						this.flagTimeTo = Time.time + (-this.speed + Mathf.Sqrt(this.speed * this.speed - 2f * Physics.gravity.y * num4)) / -Physics.gravity.y;
					}
				}
			}
		}
	...
	private int jumpCubes = 1;

	private float speed = 4f;

	private int charSizeY = 2;

	private bool isMine;

	private PathFinderArrayScript PFAS;
	...
	private Vector3 lastOccupiedPoint;

	private float lastDoorOpenTime;

	private float doorOpenDeltaTime;
}
