...
public class SurvivalRespawnScript : MonoBehaviour
{
	...
	{
		if (other.collider.gameObject.transform.root.gameObject.tag == "Player")
		{
			PlayerScript component = other.collider.gameObject.transform.root.gameObject.GetComponent<PlayerScript>();
			if (!component.dead)
			{
				...
				if (this.playerGO.GetComponent<PlayerScript>().id == Kube.IS.ps.id)
				{
				...
				else if (component.id == Kube.IS.ps.id)
				{
