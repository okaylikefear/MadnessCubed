...
public class AddTopDialog : MonoBehaviour
{
	public TopInfo info
	{
		set
		{
			...
			int index = (int)(value.roomMapNumber - (long)Kube.GPS.playerId * 20L);
	...
	private void onSend(string response)
	{
		if (response != "1")
		{
			...
			Kube.GPS.playerMoney2 -= (int)jsonData["price"];
	...
	public void onCreateClick()
	{
		...
		long roomMapNumber = (long)Kube.GPS.playerId * 20L + (long)num;
		...
		Kube.SS.Request(802, dictionary, new ServerScript.ServerCallback(this.onSend));
