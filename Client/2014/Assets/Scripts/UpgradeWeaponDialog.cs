...
public class UpgradeWeaponDialog : MonoBehaviour
{
	...
	private void ShowUpgrade()
	{
		...
		InventoryScript.WeaponParams weaponParams = Kube.IS.weaponParams[this.weaponId];
		WeaponUpgrade.getUpgradeData(this.weaponId);
		int[] array = new int[5];
		array[0] = weaponParams.currentDamageIndex;
		array[1] = weaponParams.currentAccuracyIndex;
		array[2] = weaponParams.currentDeltaShotIndex;
		array[3] = weaponParams.currentClipSizeIndex;
		int[] array2 = array;
		...
		this.title.text = weapon_upgrade_name[this.numParam] + " - уровень " + (array2[this.numParam] + 1);
	}
	...
	{
		Kube.IS.SendMessage("BuyBulletsDone", str);
	...
	public void OnUpgradeClick()
	{
		if (this.canBuy)
		{
			...
			{
				Kube.SS.UpgradeWeapon(this.weaponId, this.numParam, new ServerScript.JSONServerCallback(this.UpgradeWeaponDone));
			}
		}
		else
		{
