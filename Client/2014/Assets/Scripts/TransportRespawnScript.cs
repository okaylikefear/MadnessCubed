...
public class TransportRespawnScript : GameMapItem
{
	...
	public void SetParameters(int _x, int _y, int _z, int _type, int _state, int _respawnTime, int _healthMultiplier, int _damageMultiplier, int _id)
	{
		...
		base.transform.Find("GameObject/monstertex").renderer.material.mainTexture = Kube.ASS2.transportTex[this.type];
	}
	...
	private void setupGUI()
	{
		...
		GUI.Label(new Rect(num3 + 20f, num4 + 10f, 550f, 40f), Localize.transport_options);
		GUI.skin = Kube.ASS1.smallWhiteSkin;
		GUI.Label(new Rect(num3 + 10f, num4 + 50f, 350f, 30f), Localize.transport_type + ": " + Localize.transportName[this.type]);
