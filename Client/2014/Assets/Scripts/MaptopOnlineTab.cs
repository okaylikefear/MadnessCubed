...
public class MaptopOnlineTab : MonoBehaviour
{
	...
	public void onDayToggle()
	{
		...
		if (DayToggle.current.state != 0)
		{
			dictionary["d"] = MaptopOnlineTab.daycount[DayToggle.current.state].ToString();
		}
		Kube.SS.Request(801, dictionary, new ServerScript.ServerCallback(this.onLoaded));
	}
	...
	private void OnEnable()
	{
		...
		this.valid = false;
		Kube.SS.Request(801, null, new ServerScript.ServerCallback(this.onLoaded));
	}
	...
	private static int[] daycount = new int[]
	{
		0,
		1,
		7,
