...
public class FreeflightCamera : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			float y = base.transform.localEulerAngles.y + UnityEngine.Input.GetAxis("Mouse X") * this.mouseSensitivityX;
			this.rotY += UnityEngine.Input.GetAxis("Mouse Y") * this.mouseSensitivityY;
		...
		if (axis != 0f)
		{
			float num = (!Input.GetKey(KeyCode.LeftShift)) ? this.speedNormal : this.speedFast;
		...
		if (axis2 != 0f)
		{
			float num2 = (!Input.GetKey(KeyCode.LeftShift)) ? this.speedNormal : this.speedFast;
