			...
			if (Physics.Raycast(ray, out raycastHit, this.distance, num2))
			{
				...
				if (num3 != 0)
				{
					if (raycastHit.collider.gameObject.layer == 8)
					{
						...
						int num4 = (int)Kube.WHS.cubes[(int)vector.x, (int)vector.y, (int)vector.z].health;
		...
		for (int i = 0; i < this.rounds; i++)
		{
			if (!(this.roundsHitPoints[i] == Vector3.zero))
			{
				...
				for (int j = 0; j < array.Length; j++)
				{
					...
					{
						array[j].gameObject.rigidbody.AddForceAtPosition(this.roundsDirections[i].normalized * 10f, this.roundsHitPoints[i], ForceMode.Impulse);
					}
				}
			}
		}
