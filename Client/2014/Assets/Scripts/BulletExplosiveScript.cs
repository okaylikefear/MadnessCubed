...
public class BulletExplosiveScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		int num = 38657;
		if (Kube.GPS != null && Kube.GPS.playerId == this.dm.id_killer)
		{
			num -= 512;
		}
		...
		if (Physics.Raycast(ray, out raycastHit, this.distance, num))
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
								...
								for (int l = (int)a.z - 7; l <= (int)a.z + 7; l++)
								{
									if (j >= 0 && k >= 0 && l >= 0 && j < Kube.WHS.sizeX && k < Kube.WHS.sizeY && l < Kube.WHS.sizeZ)
									{
										if (Kube.WHS.cubes[j, k, l].type != 0)
										{
											...
											{
												int num5 = (int)Mathf.Max(0f, (float)Kube.WHS.cubes[j, k, l].health - (float)this.dm.damage * Mathf.Max(0f, 1f - num4 / (this.explosionRadius / 2f)));
												string text3 = text2;
				...
				if (array[i].gameObject.rigidbody != null)
				{
					if (this.dm.damage != 0)
					{
						array[i].gameObject.rigidbody.AddForceAtPosition(0.01f * (float)this.dm.damage * (array[i].transform.position - raycastHit.point).normalized, raycastHit.point, ForceMode.Impulse);
					}
					else
					{
						array[i].gameObject.rigidbody.AddForceAtPosition(0.5f * (array[i].transform.position - raycastHit.point).normalized, raycastHit.point, ForceMode.Impulse);
					}
				}
			}
		}
	...
	private void ForceBackRigidbodies()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (array[i].gameObject.rigidbody != null)
			{
				array[i].gameObject.rigidbody.AddForceAtPosition(this.hitDir * 10f, this.hitPos, ForceMode.Impulse);
			}
		}
	}
