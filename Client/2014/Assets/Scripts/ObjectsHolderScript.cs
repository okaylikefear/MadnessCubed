...
public class ObjectsHolderScript : MonoBehaviour
{
	...
	public ObjectsHolderScript.BuiltInMap[] findMaps(GameType gameType)
	{
		...
		for (int i = 0; i < this.builtInMaps.Length; i++)
		{
			if (this.builtInMaps[i].gameTypes[(int)gameType])
			{
	...
	public void PlayerSparks(SoundMaterialType smt, SoundHitType sht, Vector3 pos, Vector3 normal)
	{
		...
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksGlassBullet, pos, rotation);
		}
		if (smt == SoundMaterialType.ground)
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksGroundBullet, pos, rotation);
		}
		if (smt == SoundMaterialType.metal)
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksMetalBullet, pos, rotation);
		}
		if (smt == SoundMaterialType.stone)
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksStoneBullet, pos, rotation);
		}
		if (smt == SoundMaterialType.water)
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksWaterBullet, pos, rotation);
		}
		if (smt == SoundMaterialType.wood)
		{
			UnityEngine.Object.Instantiate(Kube.ASS3.sparksWoodBullet, pos, rotation);
		}
	}
	...
	private void Update()
	{
		...
		if (UnityEngine.Input.GetKeyDown(KeyCode.O) || UnityEngine.Input.GetKeyDown(KeyCode.J))
		{
			this.ShowOptions();
		}
		...
		if (Kube.ASS2 && Kube.ASS3 && Kube.ASS3.waterAnimMat != null && Time.time - this.lastWaterTexChange > this.waterAnimDeltaTime)
		{
			Kube.ASS3.waterAnimMat.mainTexture = Kube.ASS3.waterAnimTex[this.numWaterTex];
			Kube.ASS2.AAselectMat.mainTexture = Kube.ASS2.AAselectTex[this.numWaterTex];
			this.numWaterTex++;
	...
	private void OnGUI()
	{
		...
		if (this._isLoading)
		{
			GUI.DrawTexture(new Rect(0f, 0f, (float)Screen.width, (float)Screen.height), this.loadTex);
			Kube.SS.DrawLoading();
			return;
		}
		if (this.showOptions)
		{
			...
			GUI.Label(new Rect(num - 210f, num2 / 2f - 210f, 180f, 30f), "FPS:" + (float)((int)(this.fps * 10f)) / 10f);
			...
			GUI.Label(new Rect(num - 210f, num2 / 2f - 70f, 180f, 30f), string.Concat(new object[]
			{
				Localize.opt_sound,
				":",
			...
			bool flag = GUI.Toggle(new Rect(num - 190f, num2 / 2f, 180f, 60f), this.emptyScreen, Localize.opt_empty_screen);
			if (flag != this.emptyScreen)
			{
			...
			this.emptyScreen = flag;
			this.smoothMove = GUI.Toggle(new Rect(num - 190f, num2 / 2f + 70f, 180f, 60f), this.smoothMove, Localize.opt_smooth_follow);
			if (GUI.Button(new Rect(num - 210f, num2 / 2f + 210f, 180f, 30f), Localize.opt_close))
			{
				this.showOptions = false;
				Screen.lockCursor = this.tempLockCursor;
				if (Kube.IS.ps != null)
				{
		...
		if (this._menuDraw != null)
		{
			...
			{
				for (int j = this._menuBottom; j < this._menuStack.Count - 1; j++)
				{
					GUI.color = new Color(1f, 1f, 1f, 2f);
					this._menuStack[j].draw();
				}
			}
			GUI.enabled = true;
			...
			{
				GUI.DrawTexture(new Rect(0f, 0f, (float)Screen.width, (float)Screen.height), KUI.BlackTx);
			}
			this._menuDraw.draw();
	...
	private void GetPlayerMoneyDone(string str)
	{
	...
	private void SendLevelDoneDone(string str)
	{
		char[] separator = new char[]
		{
			'^'
		...
		{
			Kube.GPS.playerExp = Convert.ToInt32(array[2]);
			Kube.GPS.playerFrags = Convert.ToInt32(array[3]);
			Kube.GPS.playerMoney1 = Convert.ToInt32(array[4]);
			Kube.GPS.playerLevel = Convert.ToInt32(array[5]);
		}
		else
		{
	...
	private void GotFriends(string str)
	{
		...
		dictionary["ids"] = str;
		Kube.SS.Request(900, dictionary, new ServerScript.ServerCallback(this.onFriendsIds));
	}
	...
	private void onFriendsIds(string str)
	{
		if (str == string.Empty)
		{
			if (Kube.SN.platform == SocialNet.platformType.vk)
			{
				for (int i = 0; i < this.friends.Length; i++)
				{
	...
	private void LoadGameLevel()
	{
		...
		if (!PhotonNetwork.offlineMode)
		{
			...
			this.tempMap.NewMapType = (int)PhotonNetwork.room.customProperties["b"];
		}
	...
	{
		Kube.SS.DownloadGameData();
		Kube.SS.require("Assets2");
		Kube.SS.require("Assets5");
		Kube.SS.require("Assets6");
		for (;;)
		{
