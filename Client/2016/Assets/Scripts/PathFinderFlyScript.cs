...
public class PathFinderFlyScript : PathFinderMoveScript
{
	...
	public override void WalkingFollowTarget(Vector3 targetPos)
	{
		...
		{
			int num = Mathf.RoundToInt(this.lastOccupiedPoint.x);
			int num2 = Mathf.RoundToInt(this.lastOccupiedPoint.y);
			int num3 = Mathf.RoundToInt(this.lastOccupiedPoint.z);
			Kube.WHS.isOccupied[num, num2, num3] = false;
			this.FindPathToVector3Fly(targetPos);
			if (this.pathLength == 0)
			{
				num = Mathf.RoundToInt(this.lastOccupiedPoint.x);
				num2 = Mathf.RoundToInt(this.lastOccupiedPoint.y);
				num3 = Mathf.RoundToInt(this.lastOccupiedPoint.z);
