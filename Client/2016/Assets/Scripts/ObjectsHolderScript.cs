...
public class ObjectsHolderScript : MonoBehaviour
{
	...
	private void BICMAP()
	{
		...
		ObjectsHolderScript.BuiltInMap[] array = (ObjectsHolderScript.BuiltInMap[])this.builtInMaps.Clone();
		Array.Resize<ObjectsHolderScript.BuiltInMap>(ref array, array.Length + assetsScript.newMapTypeTex.Length);
		for (int i = 0 + this.builtInMaps.Length; i < array.Length; i++)
		{
			array[i] = new ObjectsHolderScript.BuiltInMap();
			array[i].gameTypes = new bool[7];
			array[i].Id = 6 + i;
			array[i].name = Localize.newMapTypeName[i - this.builtInMaps.Length];
			array[i].gameTypes[1] = true;
		}
		this.builtInMaps = array;
	}
	...
	private void OnGUI()
	{
		...
		if (this.showOptions)
		{
			...
			if (GUI.Button(new Rect(num - 210f, num2 / 2f + 210f, 180f, 30f), Localize.opt_close))
			{
				...
				KubeScreen.lockCursor = this.tempLockCursor;
				if (Kube.IS.ps != null)
				{
	...
	public int GetLevel(uint exp)
	{
		...
		while (this.GetExp(num) > exp || this.GetExp(num + 1) <= exp)
		{
			num++;
		}
		return num;
	...
	{
		return (int)(this.GetExp(currentLevel + 1) - this.GetExp(currentLevel));
	}

	public int GetExpFromLevelUp(uint exp, int currentLevel = -1)
	{
		if (currentLevel == -1)
		{
			currentLevel = this.GetLevel(exp);
		}
		return (int)(exp - this.GetExp(currentLevel));
	}
	...
	public float GetExpToLevelUpAlpha(uint exp, int currentLevel = -1)
	{
		...
		return (float)this.GetExpFromLevelUp(exp, currentLevel) / (float)this.GetExpToLevelUp(currentLevel);
	}
