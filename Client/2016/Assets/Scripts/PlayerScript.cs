...
public class PlayerScript : Pawn
{
	...
	private void Start()
	{
		...
		base.GetComponent<Animation>()[this.animSwordAttack[0]].layer = 5;
		base.GetComponent<Animation>()[this.animSwordAttack[1]].layer = 5;
		base.GetComponent<Animation>()[this.animSwordAttack[2]].layer = 5;
		base.GetComponent<Animation>()[this.animSwordAttack[0]].speed = 2.2f;
		base.GetComponent<Animation>()[this.animSwordAttack[1]].speed = 2.2f;
		base.GetComponent<Animation>()[this.animSwordAttack[2]].speed = 2.2f;
		base.GetComponent<Animation>()[this.animIdleEmpty].speed = 0.5f;
		base.GetComponent<Animation>()[this.animIdleSword].speed = 0.5f;
		base.GetComponent<Animation>()[this.animIdleWeapon].speed = 0.5f;
		base.GetComponent<Animation>()[this.animSwordAttack[0]].AddMixingTransform(this._neck);
		base.GetComponent<Animation>()[this.animSwordAttack[1]].AddMixingTransform(this._neck);
		base.GetComponent<Animation>()[this.animSwordAttack[2]].AddMixingTransform(this._neck);
		base.GetComponent<Animation>()[this.animWeaponShoot].layer = 5;
		base.GetComponent<Animation>()[this.animWeaponShoot].AddMixingTransform(this._neck);
		...
		for (int j = 0; j < this.weaponAnim1face.Length; j++)
		{
			...
			{
				base.GetComponent<Animation>()[this.weaponAnim1face[j]].AddMixingTransform(base.transform.Find("CameraObj"));
				base.GetComponent<Animation>()[this.weaponAnim1face[j]].layer = 10;
				base.GetComponent<Animation>()[this.weaponAnim1face[j]].speed = 1f;
			}
		}
		for (int k = 0; k < this.animDecor.Length; k++)
		{
			...
			{
				base.GetComponent<Animation>()[this.animDecor[k]].layer = 20;
			}
		}
		this.canBuild = false;
		...
		if (PhotonNetwork.offlineMode || base.photonView.isMine)
		{
			this.jetPackAwail = Kube.BCS.jetPackAwail;
		...
		if (PhotonNetwork.offlineMode || base.photonView.isMine)
		{
			...
			this.sn = Kube.SN.platform.ToString();
	...
	private void Spawn()
	{
		...
		if (Kube.BCS.gameType != GameType.creating)
		{
			for (int i = 0; i < this._inventory.Length; i++)
			{
				...
				{
					this.SelectWeapon(this._inventory[i].Num);
					break;
				}
			}
		}
	...
	private void DeadUpdate()
	{
		...
		if (!this.paused)
		{
			...
			float axis2 = KubeInput.GetAxis("Vertical");
			if (axis2 < -0.2f)
			{
		...
		if (Kube.BCS.gameType != GameType.mission)
		{
			...
			{
				this.velocity += this.cameraComp.transform.TransformDirection(Vector3.forward);
			}
			...
			if (this.controlLeft)
			{
				this.velocity -= this.cameraComp.transform.TransformDirection(Vector3.right);
			}
		...
		if (flag && this.type == 0 && Kube.BCS.gameType != GameType.survival)
		{
			if (Kube.BCS.gameType == GameType.mission)
			{
				if (this._canRespawn)
	...
	private void ReloadGun()
	{
		...
		if (this.currentWeapon != -1 && Kube.IS.weaponParams[this.currentWeapon].UsingBullets > 0 && this.clips[this.currentWeapon] < Kube.IS.weaponParams[this.currentWeapon].clipSize[Kube.IS.weaponParams[this.currentWeapon].currentClipSizeIndex] && this.bullets[Kube.IS.weaponParams[this.currentWeapon].BulletsType] > 0)
		{
			...
			base.GetComponent<Animation>().CrossFade(this.weaponRechargeBeginAnim, 0.05f);
	...
	private void LocalStart()
	{
		...
		else
		{
			...
			for (int i = 11; i < 11 + PlayerScript.geometryIdsCodes.Length - 2; i++)
			{
	...
	private void LocalUpdate()
	{
		...
		{
			this.jetPackAwail = !this.jetPackAwail;
		}
		...
		else
		{
			...
			if (this.jetPackAwail && !this.jetPackOn)
			{
			...
			else if (!this.jetPackAwail)
			{
				this.jetPackOn = false;
			}
			if (this.controlForward)
			{
				this.forwardRun = Mathf.Lerp(this.forwardRun, this.runSpeed, Time.time * 20f);
			}
			...
			if (this.controlLeft)
			{
				this.sideRun = Mathf.Lerp(this.sideRun, -this.runSpeed, Time.time * 20f);
			}
			...
			if (this.typePhys == CubePhys.air || this.typePhys == CubePhys.solid)
			{
				...
				bool flag2 = this.jetPackAwail && Kube.BCS.gameType == GameType.creating && Kube.GPS.inventarSpecItems[10] >= 0;
				...
				else
				{
					...
					if (num5 > 0f)
					{
						...
						if (this.grounded)
						{
							num6 = num3 * this.GROUND_ACCELERATE * Time.deltaTime;
						}
						else if (this.jetPackWork)
						{
							num6 = num3 * 4f * Time.deltaTime;
						}
						else
						{
							num6 = num3 * 2.5f * Time.deltaTime;
						}
						if (num6 > num5)
						{
							num6 = num5;
			...
			if (Kube.BCS.gameType == GameType.creating)
			{
				Ray ray = this.cameraComp.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f));
				int layerMask = 273;
				int num8 = Kube.GPS.fastInventar[Kube.IS.chosenFastInventar].Type;
				if (num8 != 0)
				{
					layerMask = 8465;
				}
			...
			if (!this.isDriveTransport && this.moveItem && (KubeInput.GetButtonDown("Fire1") || KubeInput.GetButtonDown("Fire2")))
			{
				...
				if (Physics.Raycast(ray2, out raycastHit2, 10f, 8465))
				{
			...
			if (!this.isDriveTransport && Physics.Raycast(ray3, out raycastHit3, 10f, layerMask2))
			{
				if (raycastHit3.collider.gameObject.transform.root.gameObject.layer == 13)
				{
					...
					bool flag4 = raycastHit3.collider.gameObject.layer != 14 || Kube.BCS.gameType == GameType.creating;
					if (this.canBuild && Kube.BCS.canChangeWorld && component3.canTake && flag4 && Kube.BCS.gameType == GameType.creating)
					{
					...
					if (component3 && component3.canActivate)
					{
						if (this.guiItemText.Length != 0)
						{
							this.guiItemText += "\n";
			...
			if (this.isDriveTransport && KubeInput.GetKeyDown(KeyCode.F))
			{
			...
			if (Kube.BCS.gameType != GameType.creating)
			{
				num9 = this._inventory[Kube.IS.chosenFastInventar].Type;
				num10 = this._inventory[Kube.IS.chosenFastInventar].Num;
			}
			else
			{
				num9 = Kube.GPS.fastInventar[Kube.IS.chosenFastInventar].Type;
				num10 = Kube.GPS.fastInventar[Kube.IS.chosenFastInventar].Num;
			}
			bool flag5 = false;
			if (KubeInput.GetButtonDown("Fire2") && !this.paused)
			{
				if (num9 == 1)
				{
					ItemPropsScript component6 = Kube.IS.gameItemsGO[num10].GetComponent<ItemPropsScript>();
					if (component6.magic)
					{
						...
						{
							this.NO.CreateMagic(num10, base.transform.position + Vector3.up * 1.5f + base.transform.TransformDirection(Vector3.forward * 0.7f), raycastHit4.point, this.onlineId);
							if (Kube.IS.UseItem(num10) == 1)
							{
							}
							flag5 = true;
						}
				...
				else if (num9 == 3)
				{
					if (Kube.BCS.gameType == GameType.creating)
					{
						this.DoUseMagic(num10);
						flag5 = true;
					}
				}
				else if (num9 == 4 && num10 == 0)
				{
					num9 = 0;
					num10 = 5;
					flag5 = false;
			...
			if (KubeInput.GetAxis("Fire2") > 0f && !this.paused)
			{
				...
				{
					if (this.currentWeapon == 11 || this.currentWeapon == 23 || this.currentWeapon == 31 || Kube.IS.weaponParams[this.currentWeapon].sniper)
					{
						this.rifleAim = true;
			...
			if (!this.isDriveTransport && KubeInput.GetAxis("Fire2") > 0f && !this.paused && !flag5 && Kube.BCS.gameType == GameType.creating && this._nextCreatingRepeat < Time.time)
			{
				this._nextCreatingRepeat = Time.time + 0.6f;
				bool flag6 = this.canBuildBlock || this.canBuild;
				if ((num9 == 0 || num9 == 1 || num9 == 3) && !flag6)
				{
					Kube.GPS.printMessage(Localize.cant_build_ask_admin, Color.yellow);
				}
				else if ((num9 == 0 || num9 == 1 || num9 == 3) && !Kube.BCS.canChangeWorld)
				{
				...
				else if (num9 == 0)
				{
					...
					if (Physics.Raycast(ray5, out rch, 10f, 8465))
					{
						...
						if (cubeFill3 != 0)
						{
							...
							{
								this.PlaceNewCube(vector4, num10, 0);
								this._nextCreatingRepeat = Time.time + 0.6f;
							}
							else
							{
						...
						else if (Vector3.Distance(vector4, base.transform.position + Vector3.up) > 1.5f)
						{
							...
							{
								this.PlaceNewCube(vector4, num10, (int)b);
								this._nextCreatingRepeat = Time.time + 0.6f;
							}
							else
							{
				...
				else if (num9 == 1 || num9 == 3)
				{
					Ray ray6 = this.cameraComp.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f));
					RaycastHit raycastHit5;
					if (Physics.Raycast(ray6, out raycastHit5, 10f, 8465))
					{
						ItemPropsScript component7 = Kube.IS.gameItemsGO[num10].GetComponent<ItemPropsScript>();
						GameMapItem component8 = Kube.IS.gameItemsGO[num10].GetComponent<GameMapItem>();
						...
						{
							ItemPropsScript component9 = raycastHit5.collider.gameObject.GetComponent<ItemPropsScript>();
							if (!component8 || component8.CanPlace(component9.gameObject.transform.position))
							{
								this.NO.CreateGameItem(num10, 0, Mathf.RoundToInt(component9.gameObject.transform.position.x), Mathf.RoundToInt(component9.gameObject.transform.position.y), Mathf.RoundToInt(component9.gameObject.transform.position.z), this.onlineId);
								BattleControllerScript bcs = Kube.BCS;
						...
						{
							Vector3 vector5 = new Vector3(Mathf.Round(raycastHit5.point.x + raycastHit5.normal.x * 0.02f), Mathf.Round(raycastHit5.point.y + raycastHit5.normal.y * 0.02f), Mathf.Round(raycastHit5.point.z + raycastHit5.normal.z * 0.02f));
							...
							if (flag7)
							{
								if (component7.buildMagic && !component7.magic)
								{
									...
									this.NO.CreateGameItem(num10, (byte)num11, Mathf.RoundToInt(vector5.x), Mathf.RoundToInt(vector5.y), Mathf.RoundToInt(vector5.z), this.onlineId);
								...
								else if (!component7.magic)
								{
									...
									if (component7.placeType == ItemPlaceType.fourRotations)
									{
										...
										if (num12 != -1)
										{
											this.NO.CreateGameItem(num10, (byte)num12, Mathf.RoundToInt(vector5.x), Mathf.RoundToInt(vector5.y), Mathf.RoundToInt(vector5.z), this.onlineId);
									...
									else if (component7.placeType == ItemPlaceType.onTheCeil)
									{
										...
										if (num13 != -1)
										{
											this.NO.CreateGameItem(num10, (byte)num13, Mathf.RoundToInt(vector5.x), Mathf.RoundToInt(vector5.y), Mathf.RoundToInt(vector5.z), this.onlineId);
									...
									{
										this.NO.CreateGameItem(num10, 0, Mathf.RoundToInt(vector5.x), Mathf.RoundToInt(vector5.y), Mathf.RoundToInt(vector5.z), this.onlineId);
			...
			if (KubeInput.GetAxis("Fire1") > 0f && !this.paused && !this.moveItem)
			{
				bool flag8 = Kube.BCS.gameType == GameType.creating;
				if (!this.canBuild && flag8)
				{
					Kube.GPS.printMessage(Localize.cant_build_ask_admin, Color.yellow);
				}
				else if (!Kube.BCS.canChangeWorld && flag8)
				{
					Kube.GPS.printMessage(Localize.cant_change_world, Color.yellow);
				}
				else if (flag8 && this._nextCreatingRepeat < Time.time)
				{
					Ray ray7 = this.cameraComp.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f));
					RaycastHit raycastHit6;
					if (Physics.Raycast(ray7, out raycastHit6, 10f, 273))
					{
						Vector3 pos = new Vector3(Mathf.Round(raycastHit6.point.x - raycastHit6.normal.x * 0.02f), Mathf.Round(raycastHit6.point.y - raycastHit6.normal.y * 0.02f), Mathf.Round(raycastHit6.point.z - raycastHit6.normal.z * 0.02f));
						if (raycastHit6.collider.gameObject.layer == 8 || raycastHit6.collider.gameObject.layer == 4)
						{
				...
				else if (this.currentWeapon != -1 && flag && !this.rechargingWeapon && Time.time - this.lastShotTimeNew[this.currentWeapon] >= Kube.IS.weaponParams[this.currentWeapon].DeltaShot)
				{
					...
					{
						int num14 = this.clips[this.currentWeapon];
						PlayerScript.Clips clips2;
						PlayerScript.Clips clips = clips2 = this.clips;
						int num15;
						int index = num15 = this.currentWeapon;
						num15 = clips2[num15];
						clips[index] = num15 - Kube.IS.weaponParams[this.currentWeapon].UsingBullets;
						...
						if (num14 - Kube.IS.weaponParams[this.currentWeapon].UsingBullets != this.clips[this.currentWeapon])
						{
			...
			if (this.rechargingWeapon && Time.time > this.rechargingWeaponStart + Kube.IS.weaponParams[this.rechargingWeaponType].reloadTime[Kube.IS.weaponParams[this.rechargingWeaponType].currentReloadTimeIndex])
			{
				...
				base.GetComponent<Animation>().CrossFade(this.weaponRechargeEndAnim, 0.05f);
				int num16 = Kube.IS.weaponParams[this.rechargingWeaponType].clipSize[Kube.IS.weaponParams[this.rechargingWeaponType].currentClipSizeIndex] - this.clips[this.rechargingWeaponType];
				num16 = Mathf.Min(num16, this.bullets[Kube.IS.weaponParams[this.rechargingWeaponType].BulletsType]);
				PlayerScript.Clips clips4;
				PlayerScript.Clips clips3 = clips4 = this.clips;
				int num15;
				int index2 = num15 = this.rechargingWeaponType;
				num15 = clips4[num15];
				clips3[index2] = num15 + num16;
				PlayerScript.Bullets bullets2;
				PlayerScript.Bullets bullets = bullets2 = this.bullets;
				int index3 = num15 = Kube.IS.weaponParams[this.rechargingWeaponType].BulletsType;
				num15 = bullets2[num15];
				bullets[index3] = num15 - num16;
			}
				...
				if (cubePhysType2 == CubePhys.solid || cubePhysType2 == CubePhys.ledder)
				{
					...
					if (direction.magnitude > 0.5f)
					{
						if (Mathf.Abs(direction.z) > Mathf.Abs(direction.x) * 0.8f)
						{
							if (this.currentWeapon == -1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunEmpty);
								base.GetComponent<Animation>()[this.animRunEmpty].speed = direction.z / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 0)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunSword);
								base.GetComponent<Animation>()[this.animRunSword].speed = direction.z / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunWeapon);
								base.GetComponent<Animation>()[this.animRunWeapon].speed = direction.z / 5f;
							}
						}
						else if (direction.x < 0f)
						{
							if (this.currentWeapon == -1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunLeftEmpty);
								base.GetComponent<Animation>()[this.animRunLeftEmpty].speed = -direction.x / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 0)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunLeftSword);
								base.GetComponent<Animation>()[this.animRunLeftSword].speed = -direction.x / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunLeftWeapon);
								base.GetComponent<Animation>()[this.animRunLeftWeapon].speed = -direction.x / 5f;
							}
						}
						else if (direction.x > 0f)
						{
							if (this.currentWeapon == -1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunRightEmpty);
								base.GetComponent<Animation>()[this.animRunRightEmpty].speed = direction.x / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 0)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunRightSword);
								base.GetComponent<Animation>()[this.animRunRightSword].speed = direction.x / 5f;
							}
							else if (Kube.IS.weaponParams[this.currentWeapon].Type == 1)
							{
								base.GetComponent<Animation>().CrossFade(this.animRunRightWeapon);
								base.GetComponent<Animation>()[this.animRunRightWeapon].speed = direction.x / 5f;
							}
						}
					}
					else if (this.currentWeapon == -1)
					{
						base.GetComponent<Animation>().CrossFade(this.animIdleEmpty);
					}
					else if (Kube.IS.weaponParams[this.currentWeapon].Type == 0)
					{
						base.GetComponent<Animation>().CrossFade(this.animIdleSword);
					}
					else if (Kube.IS.weaponParams[this.currentWeapon].Type == 1)
					{
						base.GetComponent<Animation>().CrossFade(this.animIdleWeapon);
					}
				}
	...
	private void _CreateShot(Vector3 rayOrigin, Vector3 rayDirection, int numWeapon, PhotonMessageInfo info)
	{
		...
		{
			this.weaponGOScript.PrimaryAttack(numWeapon, shotPoint, damageMessage, base.GetComponent<Animation>());
		}
		else
		{
		...
		{
			base.GetComponent<Animation>().CrossFade(this.animSwordAttack[UnityEngine.Random.Range(0, this.animSwordAttack.Length)], 0.1f);
		}
	...
	private void DrawAims()
	{
		...
		if (Kube.BCS.gameType != GameType.creating)
		{
			if (this.rifleAim && (this.currentWeapon == 11 || flag))
			{
		...
		{
			num3 = this._inventory[Kube.IS.chosenFastInventar].Type;
			int num4 = this._inventory[Kube.IS.chosenFastInventar].Num;
		}
		else
		{
	...
	private void _PlayActivity(int numActivity, PhotonMessageInfo info)
	{
		if (numActivity == 1)
		{
			base.GetComponent<Animation>().Play(this.animDecor[0]);
		}
		else if (numActivity == 2)
		{
			base.GetComponent<Animation>().Play(this.animDecor[1]);
		}
		else if (numActivity == 3)
		{
			base.GetComponent<Animation>().Play(this.animDecor[2]);
		}
		else if (numActivity == 4)
		{
			base.GetComponent<Animation>().Play(this.animDecor[3]);
		}
		else if (numActivity == 5)
		{
			base.GetComponent<Animation>().Play(this.animDecor[4]);
		}
		else if (numActivity == 6)
		{
			base.GetComponent<Animation>().Play(this.animDecor[5]);
		}
		else if (numActivity == 7)
		{
			base.GetComponent<Animation>().Play(this.animDecor[6]);
		}
		else if (numActivity == 8)
		{
			base.GetComponent<Animation>().Play(this.animDecor[7]);
		}
	}
	...
	public void SelectWeapon(int numSlot)
	{
		...
		if (this.currentWeapon != -1 && Time.time - this.lastShotTimeNew[this.currentWeapon] < Kube.IS.weaponParams[this.currentWeapon].DeltaShot)
		{
			return;
		}
		if (this.currentWeapon == num)
		{
			if (this.currentWeapon == -1)
			{
				return;
			...
			if (Kube.BCS.gameType != GameType.hunger)
			{
				num = Kube.IS.findNextWeapon(this.currentWeapon, weaponGroup);
			}
			if (num == -1)
			{
				return;
	...
	{
		if (this.currentWeapon == _numWeapon && numSkin == this.currentWeaponSkin)
		{
			return;
		}
		this.rifleAim = false;
		...
		this.rechargingWeapon = false;
		if (numSkin == -1 && this.type == 0 && this.currentWeapon != -1)
		{
			numSkin = Kube.GPS.weaponsCurrentSkin[this.currentWeapon];
		}
	...
	private void RedrawWeapon()
	{
		...
		if (!this.view3face)
		{
			...
			base.GetComponent<Animation>().Rewind(this.changeWeaponAnim);
			base.GetComponent<Animation>().Play(this.changeWeaponAnim);
	...
	private void _Die(int id_killer, int myPoints, bool isHeadshot, short weaponType, short damage, PhotonMessageInfo info)
	{
		...
		for (int i = 0; i < Kube.BCS.playersInfo.Length; i++)
		{
			...
			if (!(ps == null))
			{
				...
				{
					killer = ps;
					ps.YouKilledPlayerFull(id_killer, this.onlineId, 0, myPoints, isHeadshot);
					if (base.photonView.isMine && ps.team != this.team)
					{
						this.availableCubes = Kube.GPS.maxAvailableCubes;
						if (Kube.BCS.gameType == GameType.teams)
						{
							this.NO.ChangeTeamScore(1, ps.team);
						}
					}
					break;
				}
	...
	private void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		if (PhotonNetwork.connected)
		{
			...
			else
			{
				...
				if ((int)num2 != this.currentWeapon || (int)num != this.currentWeapon)
				{
					this.ChangeWeapon((int)num, (int)num2);
				}
	...
	public ObscuredInt currentWeapon;
