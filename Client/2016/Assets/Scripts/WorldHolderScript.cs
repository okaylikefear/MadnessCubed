...
public class WorldHolderScript : MonoBehaviour
{
	...
	{
		byte b = (byte)Mathf.Max(0, (int)(this.cubes[x, y, z].sunLight - 16));
		if (b == 0)
		{
			return;
		}
		for (int i = 0; i < 6; i++)
		{
			...
			{
				if (this.cubes[num, num2, num3].sunLight < b)
				{
					...
					if (num4 != 0)
					{
						...
						this.cubes[num, num2, num3].sunLight = b;
	...
	{
		byte b = (byte)Mathf.Max(0, (int)(this.cubes[x, y, z].lightR - 16));
		byte b2 = (byte)Mathf.Max(0, (int)(this.cubes[x, y, z].lightG - 16));
		byte b3 = (byte)Mathf.Max(0, (int)(this.cubes[x, y, z].lightB - 16));
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (this.IsInWorld(num, num2, num3))
			{
				if (this.cubes[num, num2, num3].lightR < b || this.cubes[num, num2, num3].lightG < b2 || this.cubes[num, num2, num3].lightB < b3)
				{
					int num4 = (this.cubeData[num, num2, num3] != 0) ? 1 : this.cubesDrawTypes[this.cubeTypes[num, num2, num3]];
					if (num4 != 0)
					{
						this.blocksToChange[Mathf.FloorToInt((float)num / (float)this.blockSizeX), Mathf.FloorToInt((float)num2 / (float)this.blockSizeY), Mathf.FloorToInt((float)num3 / (float)this.blockSizeZ)] = true;
						this.cubes[num, num2, num3].lightR = (byte)Mathf.Max((int)this.cubes[num, num2, num3].lightR, (int)b);
						this.cubes[num, num2, num3].lightG = (byte)Mathf.Max((int)this.cubes[num, num2, num3].lightG, (int)b2);
						this.cubes[num, num2, num3].lightB = (byte)Mathf.Max((int)this.cubes[num, num2, num3].lightB, (int)b3);
			...
			if (this.IsInWorld(x, y, z))
			{
				if (this.waterLevel[x, y, z] != 0)
				{
					if (this.IsInWorld(x, y - 1, z) && (this.cubeTypes[x, y - 1, z] == 0 || (this.cubeTypes[x, y - 1, z] == 128 && this.waterLevel[x, y - 1, z] != this.maxWaterLevel)))
					{
						...
						this.cubes[x, y - 1, z].phys = this.cubePhys[this.cubeTypes[x, y - 1, z]];
					...
					else
					{
						...
						int num2 = y + 1;
						int num3 = z;
						if (this.IsInWorld(num, num2, num3) && this.cubeTypes[num, num2, num3] != 128 && this.waterLevel[x, y, z] != this.maxWaterLevel)
						{
							byte b = 0;
							int num4 = 0;
							...
							if (b <= this.waterLevel[x, y, z])
							{
								...
								int z3;
								int z2 = z3 = z;
								byte b2 = cubeWaterGrid2[x3, y3, z3];
								cubeWaterGrid[x2, y2, z2] = b2 - 1;
								this.CheckWaterBlocksToChange(x, y, z);
								...
								if (this.waterLevel[x, y, z] == 0)
								{
									...
									this.cubes[x, y, z].phys = this.cubePhys[this.cubeTypes[x, y, z]];
									this.ChangeWorldBytesCube(x, y, z, (ushort)this.cubeTypes[x, y, z], this.waterLevel[x, y, z]);
								...
								for (m = 1; m < (int)this.maxWaterLevel; m++)
								{
									...
									for (int n = x - m; n <= x + m; n++)
									{
										for (int num5 = z - m; num5 <= z + m; num5++)
										{
											...
											{
												if (this.IsInWorld(n, y, num5) && this.IsInWorld(n, y - 1, num5) && (this.cubeTypes[n, y, num5] == 0 || this.cubeTypes[n, y - 1, num5] == 128))
												{
													flag = true;
													if (this.IsInWorld(n, y - 1, num5) && (this.cubeTypes[n, y - 1, num5] == 0 || this.cubeTypes[n, y - 1, num5] == 128))
													{
														flag2 = true;
														break;
													}
							...
							if (m != 0)
							{
								...
								for (int num6 = x - m; num6 <= x + m; num6++)
								{
									for (int num7 = z - m; num7 <= z + m; num7++)
									{
										...
										{
											if (this.IsInWorld(num6, y, num7) && (this.cubeTypes[num6, y, num7] == 0 || this.cubeTypes[num6, y - 1, num7] == 128) && this.IsInWorld(num6, y - 1, num7) && (this.cubeTypes[num6, y - 1, num7] == 0 || this.cubeTypes[num6, y - 1, num7] == 128))
											{
												flag7 = true;
												if (num7 > z)
												{
								...
								num2 = y;
								num3 = z + 1;
								if (flag3 && this.IsInWorld(num, num2, num3) && (this.cubeTypes[num, num2, num3] == 0 || (this.cubeTypes[num, num2, num3] == 128 && this.waterLevel[num, num2, num3] < this.waterLevel[x, y, z])))
								{
									this.cubeTypes[num, num2, num3] = 128;
									this.cubes[num, num2, num3].phys = this.cubePhys[this.cubeTypes[num, num2, num3]];
									this.waterLevel[num, num2, num3] = this.waterLevel[x, y, z] - 1;
									this.SetWaterToCheck(num, num2, num3, false);
									this.CheckWaterBlocksToChange(num, num2, num3);
									this.ChangeWorldBytesCube(num, num2, num3, (ushort)this.cubeTypes[num, num2, num3], this.waterLevel[num, num2, num3]);
								}
								num = x;
								num2 = y;
								num3 = z - 1;
								if (flag4 && this.IsInWorld(num, num2, num3) && (this.cubeTypes[num, num2, num3] == 0 || (this.cubeTypes[num, num2, num3] == 128 && this.waterLevel[num, num2, num3] < this.waterLevel[x, y, z])))
								{
									this.cubeTypes[num, num2, num3] = 128;
									this.cubes[num, num2, num3].phys = this.cubePhys[this.cubeTypes[num, num2, num3]];
									this.waterLevel[num, num2, num3] = this.waterLevel[x, y, z] - 1;
								...
								num2 = y;
								num3 = z;
								if (flag5 && this.IsInWorld(num, num2, num3) && (this.cubeTypes[num, num2, num3] == 0 || (this.cubeTypes[num, num2, num3] == 128 && this.waterLevel[num, num2, num3] < this.waterLevel[x, y, z])))
								{
									this.cubeTypes[num, num2, num3] = 128;
									this.cubes[num, num2, num3].phys = this.cubePhys[this.cubeTypes[num, num2, num3]];
									this.waterLevel[num, num2, num3] = this.waterLevel[x, y, z] - 1;
								...
								num2 = y;
								num3 = z;
								if (flag6 && this.IsInWorld(num, num2, num3) && (this.cubeTypes[num, num2, num3] == 0 || (this.cubeTypes[num, num2, num3] == 128 && this.waterLevel[num, num2, num3] < this.waterLevel[x, y, z])))
								{
									this.cubeTypes[num, num2, num3] = 128;
									this.cubes[num, num2, num3].phys = this.cubePhys[this.cubeTypes[num, num2, num3]];
									this.waterLevel[num, num2, num3] = this.waterLevel[x, y, z] - 1;
			...
			for (int j = 0; j < this.sizeZ; j++)
			{
				...
				for (int k = 0; k < this.sizeY; k++)
				{
					if (k > num3 && k > num2)
					{
						...
						this.cubes[i, k, j].phys = this.cubePhys[this.cubeTypes[i, k, j]];
		...
		if (fullCheck)
		{
			...
			for (int num6 = num3; num6 < num; num6++)
			{
				for (int num7 = num4; num7 < num2; num7++)
				{
					...
					{
						this.LightWaveSource(num6, num9, num7);
						if (!fullCheck)
						{
						}
					}
		...
		else
		{
			...
			for (int num11 = this.numCubesLightChange - 1; num11 >= 0; num11--)
			{
				if (this.cubes[this.cubesLightChange[num11, 0], this.cubesLightChange[num11, 1], this.cubesLightChange[num11, 2]].sunLight == 0)
				{
					...
					if (b == 255 && num12 == 1)
					{
						...
						for (int num17 = this.cubesLightChange[num11, 1]; num17 >= 0; num17--)
						{
							...
							if (num21 != 0)
							{
								...
								this.LightWaveSource(num18, num19, num20);
					...
					{
						int x = (int)((byte)(this.cubesLightChange[num11, 0] + this.lightNeibours[num12, 0]));
						int y = (int)((byte)(this.cubesLightChange[num11, 1] + this.lightNeibours[num12, 1]));
						int z = (int)((byte)(this.cubesLightChange[num11, 2] + this.lightNeibours[num12, 2]));
						this.LightWaveSource(x, y, z);
					}
				}
				else
				{
							...
							for (int num26 = 0; num26 < 6; num26++)
							{
								...
								if (this.IsInWorld(num27, num28, num29))
								{
									if (this.cubes[num27, num28, num29].sunLight != 0)
									{
										...
										{
											this.LightWaveSource(num27, num28, num29);
										}
				...
				if (this.cubes[this.cubesLightChange[num11, 0], this.cubesLightChange[num11, 1], this.cubesLightChange[num11, 2]].lightR == 0 && this.cubes[this.cubesLightChange[num11, 0], this.cubesLightChange[num11, 1], this.cubesLightChange[num11, 2]].lightG == 0 && this.cubes[this.cubesLightChange[num11, 0], this.cubesLightChange[num11, 1], this.cubesLightChange[num11, 2]].lightB == 0)
				{
					...
					if (b3 != 0)
					{
						int x2 = (int)((byte)(this.cubesLightChange[num11, 0] + this.lightNeibours[num30, 0]));
						int y2 = (int)((byte)(this.cubesLightChange[num11, 1] + this.lightNeibours[num30, 1]));
						int z2 = (int)((byte)(this.cubesLightChange[num11, 2] + this.lightNeibours[num30, 2]));
						this.LightItemWaveSources(x2, y2, z2, 255);
					}
					if (b4 != 0)
					{
						int x2 = (int)((byte)(this.cubesLightChange[num11, 0] + this.lightNeibours[num31, 0]));
						int y2 = (int)((byte)(this.cubesLightChange[num11, 1] + this.lightNeibours[num31, 1]));
						int z2 = (int)((byte)(this.cubesLightChange[num11, 2] + this.lightNeibours[num31, 2]));
						this.LightItemWaveSources(x2, y2, z2, 255);
					}
					if (b5 != 0)
					{
						int x2 = (int)((byte)(this.cubesLightChange[num11, 0] + this.lightNeibours[num32, 0]));
						int y2 = (int)((byte)(this.cubesLightChange[num11, 1] + this.lightNeibours[num32, 1]));
						int z2 = (int)((byte)(this.cubesLightChange[num11, 2] + this.lightNeibours[num32, 2]));
						this.LightItemWaveSources(x2, y2, z2, 255);
					}
				}
				else
							...
							for (int num39 = 0; num39 < 6; num39++)
							{
								...
								if (this.IsInWorld(num40, num41, num42))
								{
									if (this.cubes[num40, num41, num42].lightR != 0 || this.cubes[num40, num41, num42].lightG != 0 || this.cubes[num40, num41, num42].lightB != 0)
									{
										...
										{
											this.LightItemWaveSources(num40, num41, num42, (int)b6);
										}
										else if (this.cubes[num40, num41, num42].isLight)
										{
											this.LightItemWaveSources(num40, num41, num42, 256);
	...
	public void RestoreBlockPhys(int bX, int bY, int bZ)
	{
		for (int i = bX * this.blockSizeX; i < (bX + 1) * this.blockSizeX; i++)
		{
			for (int j = bY * this.blockSizeY; j < (bY + 1) * this.blockSizeY; j++)
			{
				...
				{
					this.cubes[i, j, k].phys = this.cubePhys[this.cubeTypes[i, j, k]];
				}
			}
		}
	}
	...
	protected void ChangeCube(int cubeX, int cubeY, int cubeZ, int typeCube, int numBlocksToChange, int geom = 0)
	{
		...
		this.cubes[cubeX, cubeY, cubeZ].phys = this.cubePhys[this.cubeTypes[cubeX, cubeY, cubeZ]];
	...
	public void ChangeItemState(int id, int newState)
	{
		if (this.gameItems != null)
		{
			for (int i = 0; i < this.gameItems.Count; i++)
			{
				...
				if (value.id == id)
				{
					value.gameObject.BroadcastMessage("ChangeItemState", newState, SendMessageOptions.RequireReceiver);
	...
	public int LoadWorld(byte[] newWorldData)
	{
		...
		if (num > 3)
		{
			this.skybox = memoryStream.ReadByte();
		}
		...
		for (int i = 0; i < this.sizeX; i++)
		{
			for (int j = 0; j < this.sizeY; j++)
			{
				for (int k = 0; k < this.sizeZ; k++)
				{
					...
					if (num == 1 && (num2 & 3840) != 0 && (num2 < 275 || num2 > 281))
					{
						num2 = 0;
						this._cubegrid.set(i, j, k, 0, 0);
					}
					this.cubeTypes[i, j, k] = (int)((ushort)num2);
					this.cubeData[i, j, k] = b;
					if (num2 > 255)
					{
					...
					{
						num3 = num2 - 155;
						if (num2 < this.cubeToItem.Length)
						{
							if (this.cubeToItem[num2] == -1)
							{
								this.cubeTypes[i, j, k] = 0;
								this.cubeData[i, j, k] = 0;
								this._cubegrid.set(i, j, k, 0, 0);
							}
							else
							{
								this.cubeTypes[i, j, k] = (int)((ushort)this.itemToCube[num3]);
							}
						}
						else
						{
					...
					{
						num3 = ((Kube.OH.blockTypes[num2].type != 1) ? -1 : Kube.OH.blockTypes[num2].itemId);
					}
					if (Kube.OH.blockTypes[num2].type == 2)
					{
						num3 = Kube.OH.blockTypes[num2].itemId;
					}
					if (num3 == -1)
					{
						this.cubesDamage[i, j, k] = this.cubesHealth[this.cubeTypes[i, j, k]];
						this.cubes[i, j, k].phys = this.cubePhys[this.cubeTypes[i, j, k]];
						if (this.cubeTypes[i, j, k] == 128)
						{
							this.waterLevel[i, j, k] = b;
						}
					}
					else
					{
						byte b2 = b % 6;
						byte state = (b - b2) / 6;
						this.CreateGameItem(num3, b2, i, j, k, (int)state, i + k * 256 + j * 256 * 256, false);
					}
				}
			}
		}
		...
		for (int l = 0; l < num4; l++)
		{
			...
			if (num >= 3)
			{
				...
				byte b3 = (byte)(num8 % 6);
				byte state2 = (byte)((num8 - (int)b3) / 6);
				num9 = num5 + num7 * 256 + num6 * 256 * 256;
				this.CreateGameItem(numItem, b3, num5, num6, num7, (int)state2, num9, false);
				m = this.gameItems.Count - 1;
			}
			else
			{
			...
			while (m < this.gameItems.Count)
			{
				if (this.gameItems[m].id == num9)
				{
		...
		{
			int numItem2 = (int)binaryReader.ReadUInt16();
			GameMapItem gameMapItem = this._CreateNewMagic(numItem2);
			int count2 = (int)binaryReader.ReadByte();
			KubeStream br2 = new KubeStream(binaryReader.ReadBytes(count2));
			gameMapItem.LoadMap(br2);
		}
		for (int num10 = 0; num10 < this.monsterRespawnS.Length; num10++)
		{
			this.monsterLastDieTime[num10] = -999999f;
		}
		for (int num11 = 0; num11 < this.transportRespawnS.Length; num11++)
		{
			this.transportLastDieTime[num11] = -999999f;
		}
		this.GenerateBounds();
		this.needSaveMap = false;
		if (this.numQueuedChanges != 0)
		{
			for (int num12 = 0; num12 < this.numQueuedChanges; num12++)
			{
				if (this.queuedChanges[num12, 0] == "ChangeCubes")
				{
					this.ChangeCubes(this.queuedChanges[num12, 1], false, false);
				}
				else if (this.queuedChanges[num12, 0] == "ChangeCubesHealth")
				{
					this.ChangeCubesHealth(this.queuedChanges[num12, 1]);
				}
			}
		}
		return 0;
		...
		for (int i = 0; i < this.sizeX; i++)
		{
			for (int j = 0; j < this.sizeY; j++)
			{
				for (int k = 0; k < this.sizeZ; k++)
				{
					...
					this.cubeTypes[i, j, k] = (int)b;
					if (b2 != 0 && b < 155 && b != 128)
					{
						b2 = 0;
					}
					this._cubegrid.set(i, j, k, (int)b, (int)b2);
					this.cubeData[i, j, k] = b2;
					if (b < 155)
					{
						this.cubesDamage[i, j, k] = this.cubesHealth[this.cubeTypes[i, j, k]];
						this.cubes[i, j, k].phys = this.cubePhys[this.cubeTypes[i, j, k]];
					...
					else
					{
						...
						if (this.itemToCube[(int)(b - 155)] != 0)
						{
							b = (byte)this.itemToCube[(int)(b - 155)];
						}
						else
						{
							b = 0;
	...
	{
		Kube.OH.PlayerSparks(Kube.OH.cubesSound[this.cubeTypes[Mathf.RoundToInt(posCube.x), Mathf.RoundToInt(posCube.y), Mathf.RoundToInt(posCube.z)]], sht, pos, normal);
	}
