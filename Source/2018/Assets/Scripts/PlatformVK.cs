...
{
	public PlatformVK(PlatformWeb owner, GameObject go, string func) : base(owner)
	{
	...
	private IEnumerator _FillFriendsRating(GameObject go, string method)
	{
		...
		WWW newWWW = new WWW("https://api.vk.com/method/friends.getAppUsers?v=3.0&access_token=" + this.access_token);
		yield return newWWW;
		...
		while (nnOffset < json.Count)
		{
			...
			WWW newWWW2 = new WWW("https://api.vk.com/method/getProfiles?v=3.0&uids=" + uids + "&fields=first_name,last_name,photo&https=1");
	...
	private IEnumerator _VKTakeScreenshot_1()
	{
		...
		{
			"https://api.vk.com/method/photos.getWallUploadServer?v=3.0&uid=",
			this.playerUID,
			"&gid=",
			this.playerUID,
		...
		{
			"https://api.vk.com/method/photos.saveWallPhoto?v=3.0&uid=",
			this.playerUID,
			"&gid=",
			this.playerUID,
			"&photo=",
