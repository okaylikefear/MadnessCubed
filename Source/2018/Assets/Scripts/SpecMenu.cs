...
public class SpecMenu : ShopMenu
{
	...
	{
		if (!UIToggle.current || !UIToggle.current.value)
		{
			return;
		}
	...
	public override void onSelectKube(int kubeId)
	{
		if (this.inventoryPageType == 0)
		{
			if (kubeId != 0 && kubeId != 10)
			{
				if (Kube.GPS.inventarSpecItems[kubeId] > 0)
				{
					int t = 7;
		...
		else if (this.inventoryPageType == 1 && Kube.GPS.inventarItems[kubeId] > 0)
		{
			int t2 = 7;
			if (this.inventoryPageType == 1)
			{
			...
			this.fi.SelectSlot(new FastInventar(t2, kubeId));
			return;
		}
		this.fi.stop();
