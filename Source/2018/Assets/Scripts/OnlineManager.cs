...
public class OnlineManager : MonoBehaviour
{
	...
	{
		PhotonNetwork.PhotonServerSettings = UnityEngine.Object.Instantiate<ServerSettings>(PhotonNetwork.PhotonServerSettings);
		PhotonNetwork.PhotonServerSettings.AppID = "3bd3325c-d3df-4252-a0f3-0857c2820f12";
	...
	private void ConnectUsingSettings()
	{
		...
		PhotonNetwork.player.customProperties["sn"] = Kube.SN.sn.ToString();
		if (Kube.GPS.clan != null)
		{
			PhotonNetwork.player.customProperties["clan"] = Kube.GPS.clan.id;
		}
		try
		{
			PhotonNetwork.ConnectUsingSettings("68");
		}
		catch
		{
			this.EndAllActivity();
	...
	private IEnumerator _CreateRoom(GameType _newGameType, bool builtin, long _newGameMap, int _newGameCanBreak, int _newGameLight, bool offline, int missionId = 0, string roomPassword = "", string roomTitle = "")
	{
		...
		playingPlayersHash["sn"] = Kube.SN.sn.ToString();
		...
		{
			playingPlayersHash["pw"] = roomPassword;
		}
		PhotonNetwork.CreateRoom(roomParams + roomTitle, ro, null);
		this.creatingRoom = true;
		yield return new WaitForSeconds(1f);
		yield break;
	}
	...
	private IEnumerator _PlayMission(MissionDesc mission, bool offline)
	{
		...
		{
			yield return base.StartCoroutine(this._CreateRoom(GameType.mission, true, mission.mapId, mission.canBreak, (mission.dayTime != 0) ? 0 : 1, offline, mission.id, string.Empty, string.Empty));
		}
		while (PhotonNetwork.room == null)
		{
			yield return new WaitForSeconds(1f);
	...
	private void CreateRoomList()
	{
		...
		for (int i = 0; i < this.wholeNumRooms; i++)
		{
			...
			string name = this._rooms[i].name;
			if (name.Length <= 1)
			{
			...
			else
			{
				...
				this.rooms[i].roomPassword = string.Empty;
				if (this._rooms[i].customProperties.ContainsKey("pw"))
				{
					this.rooms[i].roomPassword = (string)this._rooms[i].customProperties["pw"];
				}
				if (name.Length > 6)
				{
					this.rooms[i].roomTitle = name.Substring(6);
				}
				string text = name.Substring(0, 6);
				if (text.Length >= 2)
				{
							...
							if (friends[j].Room == this._rooms[i].name)
							{
								for (int k = 0; k < Kube.OH.friends.Length; k++)
								{
									if (Kube.OH.friends[k].uid == friends[j].Name)
									{
										...
										{
											OnlineManager.RoomsInfo[] array = this.rooms;
											int num3 = i;
											array[num3].friendsStr = array[num3].friendsStr + ";";
										}
										OnlineManager.RoomsInfo[] array2 = this.rooms;
										int num4 = i;
										array2[num4].friendsStr = array2[num4].friendsStr + string.Empty + k;
	...
	private IEnumerator _PlayClanWar(OnlineManager.RoomsInfo room, int c0, int c1, bool canCreate)
	{
		this.popup.SetActive(true);
		this._process = OnlineManager.Process.quickPlay;
		int c1p = 0;
		...
		room.roomPassword = Kube.OH.GetServerCode((int)Kube.SN.sn, 1) + Kube.OH.GetServerCode(c0, 3) + Kube.OH.GetServerCode(c1p, 1) + Kube.OH.GetServerCode(c1, 3);
		if (!PhotonNetwork.connected)
		{
			yield return base.StartCoroutine(this._Connect());
			while (!PhotonNetwork.connected)
		...
		int numJoinAbleGames = 0;
		int[] joinAbleRooms = new int[512];
		int c0p = (int)Kube.SN.sn;
		if (this.rooms != null)
		{
			for (int i = 0; i < this.wholeNumRooms; i++)
			{
				if (this.rooms[i].roomType == 10)
				{
					string roomPassword = this.rooms[i].roomPassword;
					if (!(roomPassword == string.Empty))
					{
						int _c0 = Kube.OH.DecodeServerCode(roomPassword.Substring(1, 3));
						int _c0p = Kube.OH.DecodeServerCode(roomPassword.Substring(0, 1));
						int _c = Kube.OH.DecodeServerCode(roomPassword.Substring(5, 3));
						int _c1p = Kube.OH.DecodeServerCode(roomPassword.Substring(4, 1));
						if ((_c0 == c0 && _c0p == c0p) || (_c == c0 && _c1p == c0p) || _c == 0 || (c1 == _c && c0p == _c1p))
						{
							if (room.roomMapNumber == -1L || this.rooms[i].roomMapNumber == (long)((int)room.roomMapNumber))
							{
								if (this.rooms[i].players < this.rooms[i].maxPlayers - 1)
								{
									joinAbleRooms[numJoinAbleGames] = i;
		...
		else
		{
			...
			if (room.roomMapNumber == -1L)
			{
				room.roomMapNumber = -Kube.OH.randomMap(GameType.clanwar);
			}
			yield return base.StartCoroutine(this._CreateRoom(GameType.clanwar, true, room.roomMapNumber, 0, 0, false, 0, room.roomPassword, string.Empty));
		}
		while (PhotonNetwork.room == null)
		{
			yield return new WaitForSeconds(1f);
		}
		UnityEngine.Debug.Log("_PlayClanWar connected");
		yield break;
	}

	public void PlayClanWar(OnlineManager.RoomsInfo room, int c0, int c1, bool canCreate)
	{
		base.StartCoroutine(this._PlayClanWar(room, c0, c1, canCreate));
	}
