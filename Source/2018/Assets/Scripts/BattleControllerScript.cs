...
{
	public bool isWaintingForMap
	{
		get
		{
			return this.waintingForMap != 4;
		}
	}
	...
	private void Start()
	{
		...
		this.hud = Cub2UI.FindObject<UIHUD>();
		...
		{
			this.canChangeWorld = this.mapCanBreak;
	...
	{
		base.StartCoroutine(this._OnMapLoaded(mapData));
	}

	private IEnumerator _OnMapLoaded(byte[] mapData)
	{
		base.CancelInvoke("RequestMap");
		if (this.waintingForMap < 3)
		{
			this.waintingForMap = 3;
			yield return base.StartCoroutine(Kube.WHS.LoadWorld(mapData));
			if (Kube.WHS.LoadWorldResult == 1)
			{
	...
	private void BeginGame()
	{
		...
		this.hud.Init();
		Kube.WHS.SetDayLight((this.dayLightType != 2) ? ((float)this.dayLightType) : 1f, false);
		if (Kube.WHS.skybox == 2 && QualitySettings.GetQualityLevel() > 0)
		{
		...
		if (this.gameType == GameType.creating || this.gameType == GameType.shooter || this.gameType == GameType.test || this.gameType == GameType.survival)
		{
			...
			else if (this.gameType == GameType.creating)
			{
				this.gameTypeController = base.gameObject.AddComponent<CreatingController>();
			}
		}
		...
		else if (this.gameType == GameType.clanwar)
		{
			this.gameTypeController = base.gameObject.AddComponent<GTClanWar>();
			this.gameTypeController.Initialize();
		}
		...
		{
			this.gameTypeController.BeginGame();
		}
	...
	public void MonsterRespawnTick()
	{
		if (Time.time - this.lastCheckMonstersTime > this.lastCheckMonstersDeltaTime && PhotonNetwork.isMasterClient)
		{
			...
			for (int k = 0; k < Kube.WHS.monsterRespawnS.Length; k++)
			{
				...
				{
					if (Time.time < Kube.WHS.monsterRespawnS[k].monsterLastDieTime)
					{
						bool flag = false;
			...
			for (int m = 0; m < Kube.WHS.monsterRespawnS.Length; m++)
			{
				...
				{
					if (Time.time - Kube.WHS.monsterRespawnS[m].monsterLastDieTime > (float)Kube.WHS.monsterRespawnS[m].secToRespawn[Kube.WHS.monsterRespawnS[m].respawnTime])
					{
	...
	private void Update()
	{
		...
		else
		{
			...
			if (this.dayLightType == 2 && this.gameType != GameType.creating)
			{
			...
			if (KubeInput.GetKey(KeyCode.Tab))
			{
			...
			bool flag2 = KubeInput.GetKeyDown(KeyCode.Escape) || UnityEngine.Input.GetKeyDown(KeyCode.BackQuote);
	...
	public void SendGameResultToServer()
	{
		...
		if (this.gameType == GameType.clanwar && PhotonNetwork.isMasterClient)
		{
			this.NO.SendClanWarEnd();
		}
	}
	...
	public void EndGame(BattleControllerScript.EndGameType endGameType)
	{
		...
		Kube.OH.BroadcastMessage("EndGame", true);
