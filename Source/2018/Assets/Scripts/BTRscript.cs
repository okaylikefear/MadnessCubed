...
public class BTRscript : CarBase
{
	...
	private void DriveCar0()
	{
		...
		if (this.canonShootPoint && UnityEngine.Input.GetAxis("Fire1") > 0f && !Kube.IS.ps.paused && Time.time - this.canonLastShootTime > this.canonShootDeltaTime)
		{
	...
	private void _CreateGunShot(Vector3 rayOrigin, Vector3 rayDirection, PhotonMessageInfo info)
	{
		...
		DamageMessage damageMessage = new DamageMessage();
		if (Kube.BCS.onlineId == this.driversId[1])
		{
		...
		gameObject.transform.LookAt(worldPosition);
		gameObject.SendMessage("SetDamageParam", damageMessage);
		this.gunGun.GetComponent<AudioSource>().Play();
	}
	...
	public override void TransportUpdate(int numPlace)
	{
		...
		this.wRpm = this.meanRPM / (float)this.wheelsPhys.Length;
		this.meanRPM /= (float)this.wheelsPhys.Length * this.GearRatio[this.CurrentGear];
		for (int l = 0; l < this.wheelsPhys.Length; l++)
		{
			...
			this.wheelsModels[l].transform.localRotation = Quaternion.Euler(0f, this.wheelsPhys[l].steerAngle, this.wheelsRotateAngle[l]);
		}
		if (this.ruleGO != null)
		{
