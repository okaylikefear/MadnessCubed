...
public class BoxesMenu : ShopMenu
{
	...
	public override void onBuyKube(FastInventar fi)
	{
		...
		{
			UnboxDialog unboxDialog = Cub2UI.FindAndOpenDialog<UnboxDialog>("dialog unbox");
			unboxDialog.Open(fi.Num);
			return;
		}
		BuyItemDialog buyItemDialog = Cub2Menu.Find<BuyItemDialog>("dialog_buy_item");
		buyItemDialog.itemId = fi.Num;
		buyItemDialog.gameObject.SetActive(true);
	}
