...
public class WireScript : GameMapItem
{
	...
	public override bool CanPlace(Vector3 targetCube)
	{
		int num = Kube.WHS.GetTriggerId(Mathf.RoundToInt(targetCube.x), Mathf.RoundToInt(targetCube.y), Mathf.RoundToInt(targetCube.z));
		if (num == -1)
		{
	...
	private void SetParameters(int playerId)
	{
		...
		this.NO.CreateNewWire(this.triggerId % 256, (this.triggerId >> 8) % 256, this.delay, (int)this.targetType, this.xt, this.yt, this.zt, this.id, Kube.BCS.onlineId);
	...
	{
		this.Init();
		this.NO.SaveWire(this.triggerId % 256, (this.triggerId >> 8) % 256, this.delay, (int)this.targetType, this.xt, this.yt, this.zt, this.id, Kube.BCS.onlineId);
	}
