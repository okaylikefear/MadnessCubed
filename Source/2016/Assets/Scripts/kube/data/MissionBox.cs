	...
	public class MissionBox
	{
		...
		protected static void missionLoadDone(string response)
		{
			...
			for (int i = 0; i < jsonData2.Count; i++)
			{
				...
				string title = jsonData4["mapname"].ToString();
				if (MissionBox.missions[i].id > 0 && MissionBox.missions[i].id < Localize.mission_name.Length && Localize.mission_name[MissionBox.missions[i].id] != null && Localize.mission_name[MissionBox.missions[i].id].Length > 2)
				{
					title = Localize.mission_name[MissionBox.missions[i].id];
				}
				MissionBox.missions[i].title = title;
			...
			for (int k = 0; k < jsonData3.Count; k++)
			{
				...
				string title2 = jsonData5["name"].ToString();
				if (Localize.T("episode" + MissionBox.episodes[k].id) != null)
				{
					title2 = Localize.T("episode" + MissionBox.episodes[k].id);
				}
				MissionBox.episodes[k].title = title2;
			}
