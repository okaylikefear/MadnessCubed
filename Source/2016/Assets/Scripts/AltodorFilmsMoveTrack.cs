	...
	private void Update()
	{
		...
		for (i = 0; i < this.wayPointsTrack.Length - 1; i++)
		{
			if (num >= this.wayPointTime[i] && num <= this.wayPointTime[i + 1])
			{
				...
				b = Vector3.Lerp(this.wayPointsTrack[i].position, this.wayPointsTrack[i + 1].position, t);
				if (this.interpolateRotation)
				{
					b2 = Quaternion.Slerp(this.wayPointsTrack[i].rotation, this.wayPointsTrack[i + 1].rotation, t);
				}
				else
				{
					b2 = this.wayPointsTrack[i].rotation;
				}
				base.transform.position = Vector3.Lerp(base.transform.position, b, this.moveLerpKoeffPos);
				base.transform.rotation = Quaternion.Slerp(base.transform.rotation, b2, this.moveLerpKoeffRot);
				break;
			}
		}
