...
public class OnlineMenu : MonoBehaviour
{
	...
	public void onItemClick2()
	{
		...
		for (int i = 0; i < rooms.Length; i++)
		{
			if (!(rooms[i].roomPassword != string.Empty))
			{
				...
				{
					if (this._canBreak || rooms[i].mapCanBreak != 1)
					{
						if (rooms[i].buildInMap && rooms[i].roomMapNumber == component.room.roomMapNumber && rooms[i].roomType == component.room.roomType)
						{
							list.Add(i);
							break;
						}
		...
		if (list.Count > 0)
		{
			int num = list[UnityEngine.Random.Range(0, list.Count - 1)];
	...
	private OnlineManager.RoomsInfo[] selectRoomsGroup()
	{
		...
		for (int i = 0; i < Kube.OH.builtInMaps.Length; i++)
		{
			...
			{
				int num = 0;
				if (Kube.OH.builtInMaps[i].gameTypes[array[j]])
				{
