...
public class RocketScript : MonoBehaviour
{
	...
	protected void Start()
	{
		...
		{
			GetComponent<Rigidbody>().AddForce(base.transform.TransformDirection(Vector3.forward) * this.flySpeed * GetComponent<Rigidbody>().mass, ForceMode.Impulse);
		}
	}
	...
	private void Update()
	{
		...
		{
			this.rocketView.transform.rotation = Quaternion.LookRotation(GetComponent<Rigidbody>().velocity);
		}
	}
	...
	private void OnCollisionEnter(Collision col)
	{
		...
		for (int m = 0; m < array.Length; m++)
		{
			if (array[m].gameObject.rigidbody != null)
			{
				...
				{
					array[m].gameObject.rigidbody.AddForceAtPosition(1f * (array[m].transform.position - base.transform.position).normalized, base.transform.position, ForceMode.Impulse);
				}
			}
		}
