...
public class WeaponInfo : MonoBehaviour
{
	...
	public void ShowWeapon(int weaponId)
	{
		...
		float num = 2592000f;
		bool flag = (float)Kube.GPS.inventarWeapons[weaponId] > Time.time;
		...
		int num2 = Kube.GPS.weaponsPrice1[weaponId, 0];
		if (num2 == 0)
		{
			this.price.isGold = true;
			num2 = Kube.GPS.weaponsPrice2[weaponId, 0];
		}
		else
		{
		...
		if (!flag2)
		{
			this.price.value = num2;
		}
		for (int i = 0; i < upgradeData.upgradeAvail.Length; i++)
		{
			bool active = upgradeData.upgradeAvail[i] > upgradeData.upgradeIndex[i] + 1;
			this.prms[i].button.gameObject.SetActive(active);
		}
		...
		for (int j = 0; j < this.prms.Length; j++)
		{
			EventDelegate.Add(this.prms[j].button.onClick, new EventDelegate(new EventDelegate.Callback(this.onUpgrade)));
		}
	}
