...
public class GetWorldLightColorScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Mathf.Abs(this.color.r - this.newColor.r) > 0.02f || Mathf.Abs(this.color.g - this.newColor.g) > 0.02f || Mathf.Abs(this.color.b - this.newColor.b) > 0.02f)
		{
	...
	private void ChangeColor()
	{
		for (int i = 0; i < this.renderers.Length; i++)
		{
			...
			{
				this.renderers[i].material.color = this.color;
			}
		}
	}
