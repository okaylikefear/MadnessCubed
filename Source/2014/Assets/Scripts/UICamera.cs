...
public class UICamera : MonoBehaviour
{
			...
			if (uicamera.enabled && NGUITools.GetActive(uicamera.gameObject))
			{
				...
				if (!float.IsNaN(vector.x) && !float.IsNaN(vector.y))
				{
					if (vector.x >= 0f && vector.x <= 1f && vector.y >= 0f && vector.y <= 1f)
					{
						...
						{
							if (Physics.Raycast(ray, out UICamera.lastHit, distance, num))
							{
						...
						{
							RaycastHit[] array = Physics.RaycastAll(ray, distance, num);
							if (array.Length > 1)
							{
								int j = 0;
	...
	private void Awake()
	{
		...
		if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == (RuntimePlatform)21 || Application.platform == (RuntimePlatform)22)
		{
			this.useMouse = false;
			this.useTouch = true;
	...
	private void Update()
	{
		...
		if (this.useMouse && UICamera.mCurrentSelection != null)
		{
			...
			else if (this.cancelKey1 != KeyCode.None && UnityEngine.Input.GetKeyDown(this.cancelKey1))
			{
				UICamera.currentScheme = UICamera.ControlScheme.Controller;
		...
		{
			float num = string.IsNullOrEmpty(this.scrollAxisName) ? 0f : UnityEngine.Input.GetAxis(this.scrollAxisName);
			if (num != 0f)
			{
				UICamera.Notify(UICamera.mHover, "OnScroll", num);
			}
			if (UICamera.showTooltips && this.mTooltipTime != 0f && (this.mTooltipTime < RealTime.time || UnityEngine.Input.GetKey(KeyCode.LeftShift) || UnityEngine.Input.GetKey(KeyCode.RightShift)))
			{
	...
	public void ProcessOthers()
	{
		...
		bool flag = false;
		bool flag2 = false;
		if (this.submitKey0 != KeyCode.None && UnityEngine.Input.GetKeyDown(this.submitKey0))
		{
			UICamera.currentKey = this.submitKey0;
			flag = true;
		}
		if (this.submitKey1 != KeyCode.None && UnityEngine.Input.GetKeyDown(this.submitKey1))
		{
			UICamera.currentKey = this.submitKey1;
			flag = true;
		}
		if (this.submitKey0 != KeyCode.None && UnityEngine.Input.GetKeyUp(this.submitKey0))
		{
			UICamera.currentKey = this.submitKey0;
			flag2 = true;
		}
		if (this.submitKey1 != KeyCode.None && UnityEngine.Input.GetKeyUp(this.submitKey1))
		{
			UICamera.currentKey = this.submitKey1;
			flag2 = true;
		}
