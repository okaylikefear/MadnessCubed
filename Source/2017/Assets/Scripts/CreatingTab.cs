...
public class CreatingTab : Tab
{
	...
	public void OnTestButton()
	{
		...
		UnityEngine.Object.Destroy(UIButton.current.GetComponentInChildren<CubLocalize>());
		if (!component.value)
		{
	...
	private void Update()
	{
		...
		for (int i = 0; i < playersInfo.Length; i++)
		{
			...
			if (!this._dict.ContainsKey(serverId) || !this._dict[serverId])
			{
	...
	{
		HeadTab headTab = Cub2Menu.Find<HeadTab>("CREATING_MENU");
		Cub2Menu.instance.head.select(headTab.gameObject, false, true);
	}
