...
public class GameParamsScript : MonoBehaviour
{
	...
	public void SetLocale(string locale, bool clear = true)
	{
		...
		if (textAsset != null)
		{
			LocalizeUtils.load(textAsset.bytes, clear);
			this._locale = locale;
		}
		else
		{
	...
	private IEnumerator _SetLocale(string locale)
	{
		...
		if (www.error == null)
		{
			LocalizeUtils.load(www.bytes, true);
			this._locale = locale;
		}
		Kube.SN.PostProcessLocale();
		yield break;
	...
	public FastInventar[] fastInventar
	{
		get
		{
			...
			{
				return Kube.GPS.fastInventarCreating;
			}
	...
	public void Init()
	{
		...
		this.cubesTimeOfEnd = new ObscuredInt[6];
		int num = 250;
		...
		this.inventarSpecItems = new int[32];
		...
		this.clothesPrice = new int[128, 3];
		this.clothesBonus = new float[128, 16];
		this.upgradePrice = new PriceValue[this.inventarWeapons.Length, 6, 8];
		this.fastInventarCreating = new FastInventar[11];
		this.fastInventarWeapon = new FastInventar[11];
		for (int i = 0; i < 11; i++)
		{
			this.fastInventarCreating[i].Type = 0;
			this.fastInventarCreating[i].Num = Kube.IS.cubesNatureNums[i];
		}
		for (int j = 0; j < 11; j++)
		{
		...
		this.playerIsClothes = new int[128];
		this.playerClothes = new int[128];
	...
	public ObscuredInt[] cubesTimeOfEnd;
	...
	public int[] playerIsClothes = new int[128];
	...
	public FastInventar[] fastInventarCreating;
