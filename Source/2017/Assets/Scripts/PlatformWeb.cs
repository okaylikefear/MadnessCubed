...
public class PlatformWeb : MonoBehaviour, ISubscriptions, IPlatform
{
	...
	public Texture moneyIconTx
	{
		get
		{
			return this.moneyIcon[(int)this.sn];
		}
	}
	...
	public void InitDone()
	{
		...
		bool clear = true;
		if (this.locale != LocaleEnum.ru_RU.ToString())
		{
			Kube.GPS.SetLocale(LocaleEnum.en_US.ToString(), true);
			clear = false;
		}
		Kube.GPS.SetLocale(this.locale, clear);
	...
	public void UnityVars(string ans)
	{
		...
		this.sn = (socialNetType)((int)Enum.Parse(typeof(socialNetType), value));
		this.initPlatform();
		...
		{
			string text4 = jsonData["assets"].ToString();
			string bundlesPath = text4;
			if (jsonData.Keys.Contains("bundles"))
			{
				bundlesPath = jsonData["bundles"].ToString();
			}
			resourceManager.Init(text4, bundlesPath);
		}
	...
	{
		Kube.SS.Request(23, null, delegate(string responce)
		{
			Kube.IS.gameObject.SendMessage("BuyVIPDone");
		});
	}

	public void OnOrderSuccess2(string payment_id)
	{
		if (this.sn == socialNetType.fb)
		{
	...
	public void OnGiftSuccess(string uid)
	{
		Kube.SS.Request(3841, uid, delegate(string ans)
		{
	...
	public void ShowGift(string uid)
	{
		Application.ExternalCall("ShowGift", new object[]
		{
			uid
		});
	}
	...
	{
		Application.ExternalCall("ShowFreeGold", new object[0]);
	}

	public T Get<T>() where T : class
	{
		if (this._platform == socialNetType.vk && typeof(ISubscriptions) == typeof(T))
		{
			return this as T;
		}
	...
	public FriendRecord playerInfo
	{
		get
		{
	...
	public bool hasFreeGold
	{
		get
		{
			return this._platform == socialNetType.vk;
		}
	}

	public void BuySubs()
	{
		Application.ExternalCall("ShowSubsVIP", new object[0]);
	}

	[SerializeField]
	private socialNetType _platform;
	...
	private FriendRecord _playerInfo;

	private ServerCallback _buyVIPSubsDone;
}
