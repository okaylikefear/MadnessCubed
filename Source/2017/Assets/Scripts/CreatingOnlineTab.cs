...
public class CreatingOnlineTab : MonoBehaviour
{
	...
	private void Invalidate()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			else if (array[i].buildInMap && (long)Localize.buildinMapName.Length > array[i].roomMapNumber)
			{
	...
	private OnlineManager.RoomsInfo[] selectRooms()
	{
		...
		{
			if (this._onlyFriends || !(rooms[i].roomPassword != string.Empty))
			{
				if (!this._onlyFriends || rooms[i].gameWithFriends)
				{
					if (rooms[i].roomType == 1)
					{
						if (!string.IsNullOrEmpty(rooms[i].sn) && !(rooms[i].sn != b))
						{
							list.Add(rooms[i]);
						}
					}
