...
public class FastInventarPanel : MonoBehaviour
{
	...
	public virtual void CurrentSlot(int index)
	{
	...
	private void _SelectSlot(FastInventar item)
	{
		...
		this.arrows.GetComponent<Animation>().Play();
	...
	public void onSlotClick()
	{
		if (this.ingame)
		{
			this.onSlotClickGame();
		}
		else
		{
			this.onSlotClickMenu();
		}
	}

	private void onSlotClickGame()
	{
		SlotItem current = SlotItem.current;
		int num = Array.IndexOf<SlotItem>(this.slots, current);
		Kube.IS.ChoseFastInventar(num);
	}

	private void onSlotClickMenu()
	{
		SlotItem current = SlotItem.current;
		int num = Array.IndexOf<SlotItem>(this.slots, current);
		...
		this.currentItem = FastInventarPanel.empty;
		int type = (this.fastInventar != Kube.GPS.fastInventarCreating) ? 1 : 0;
