			...
			if (newState == 0)
			{
				...
				for (int i = 0; i < (int)this.IPS.doorSize.x; i++)
				{
					for (int j = 0; j < (int)this.IPS.doorSize.y; j++)
					{
						...
						Kube.WHS.prop[Mathf.RoundToInt(base.transform.position.x + point.x), Mathf.RoundToInt(base.transform.position.y + point.y), Mathf.RoundToInt(base.transform.position.z + point.z)] = 1;
					}
				}
	...
	private void ClearWorldProps()
	{
		for (int i = 0; i < (int)this.IPS.doorSize.x; i++)
		{
			for (int j = 0; j < (int)this.IPS.doorSize.y; j++)
			{
				Vector3 point = new Vector3(0f, (float)j, (float)i);
				point = base.transform.rotation * point;
				Kube.WHS.prop[Mathf.RoundToInt(base.transform.position.x + point.x), Mathf.RoundToInt(base.transform.position.y + point.y), Mathf.RoundToInt(base.transform.position.z + point.z)] = 0;
			}
		}
	}
